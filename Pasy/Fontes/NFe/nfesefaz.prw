	...
	For nCountNF := 1 To Len(aCupRefLoj)
		...
		//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
						...
						EndIf
							...
							If (cAliasSD2)->D2_CLIENTE == "000008"
								dbSelectArea("SZ2")
								dbSetOrder(6)
						...
						IF((cAliasSD2)->D2_TIPO == "D" .And. !lIpiDev) .Or. lConsig .Or. (Alltrim((cAliasSD2)->D2_CF) $ cMVCFOPREM ) .or. ((cAliasSD2)->D2_TIPO == "B" .and. lIpiBenef) .or. ((cAliasSD2)->D2_TIPO=="P" .And. lComplDev .And. !lIpiDev)
							If (cVerAmb >= "4.00" .And. (cAliasSD2)->D2_TIPO == "D" .And. lIpiDev .and. cTPNota == "4")
								nValOutr +=(cAliasSD2)->D2_VALIPI
							ElseIf cVerAmb >= "4.00" .And. cTPNota == "4"
								nValOutr += 0
							Else
								nValOutr +=(cAliasSD2)->D2_VALIPI
							EndIf
						...
						aAdd(aInfoItem,{(cAliasSD2)->D2_PEDIDO,(cAliasSD2)->D2_ITEMPV,(cAliasSD2)->D2_TES,(cAliasSD2)->D2_ITEM})
							...
							If _lExport
								...
								If dbSeek(xFilial("EE7")+SC5->C5_PEDEXP)
									...
									If dbSeek(xFilial("EE8")+SC5->C5_PEDEXP + SC6->C6_ITEM )
										...
										/// TRECHO ABAIXO COMENTADO EM 19/10/10 DEVIDO GERAR ERRO NO MOMENTO DA TRANSMISSÃO DA NFE
							...
							Else
								...
								_nPos := AScan( _aPedVen, SC5->C5_NUM )
							...
							_cDesc := Alltrim(_cDESCSZ2)+" - "+RTrim(SC6->C6_CPROCLI)+" PED.CLIENTE: "+Alltrim(SC6->C6_PEDCLI)+" PED.PASY+IT :"+SC6->C6_NUM+SC6->C6_ITEM
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
		       ...
		       EndIf
				...
				If (((cAliasSD1)->D1_TIPO == "D" .And. !lEipiDev) .Or. ((cAliasSD1)->D1_TIPO == "B" .and. lIpiBenef))
					If (cVerAmb >= "4.00" .And. (cAliasSD1)->D1_TIPO == "D" .And. lEipiDev .and. cTPNota == "4")
				...
				aadd(aProd,	{Len(aProd)+1,;
					...
					SB1->B1_UM,;
					(cAliasSD1)->D1_QUANT,;
					IIF(!((cAliasSD1)->D1_TIPO $"IP" .Or. ((cAliasSD1)->D1_TIPO $ "D" .And. Len(aNfVinc[nCountIT]) > 9 .And. aNfVinc[nCountIT][10] == "P")) ,(cAliasSD1)->D1_TOTAL,0),;
	           ...
	           //Tratamento para que o valor de PIS ST e COFINS ST venha a compor o valor total da tag vOutros  (NT 2011/004). E devolução de compra com IPI não tributado apenas para saida
				...
				Else
					...
					+ IIF(((cAliasSD1)->D1_TIPO $"IP" .Or. ((cAliasSD1)->D1_TIPO == "D" .And. Len(aNfVinc[nCountIT]) > 9 .And. aNfVinc[nCountIT][10] == "P")),0,(cAliasSD1)->D1_VALIPI)+(cAliasSD1)->D1_ICMSRET + (cAliasSD1)->D1_VALPS3 + (cAliasSD1)->D1_VALCF3;
...
If !Empty(aNota)
	...
	For nX := 1 To Len(aProd)
		...
		cString += 	NfeItem(aProd[nX],aICMS[nX],aICMSST[nX],aIPI[nX],aPIS[nX],aPISST[nX],aCOFINS[nX],aCOFINSST[nX],aISSQN[nX],aCST[nX],;
					...
					aAgrPis[nX],aAgrCofins[nX],nIcmsDif,aICMUFDest[nX],@nvFCPUFDest,@nvICMSUFDest,@nvICMSUFRemet,cAmbiente,aIPIDevol[nX],@nvBCUFDest, aItemVinc[nX], @npFCPUFDest,@npICMSUFDest,@npICMSInter,@npICMSIntP,aLote[nX],aInfPr[nX])
	Next nX
	...
	cString += NfeInfAd(cMensCli,cMensFis,aPedido,aExp,cAnfavea,aMotivoCont,aNota,aNfVinc,aProd,aDI,aNfVincRur,aRetido,cNfRefcup,cSerRefcupc,cTipo,nIPIConsig,nSTConsig,lBrinde,cVerAmb,Iif(aNota[5] == "D",aRefECF,{}),nVicmsDeson,nvFCPUFDest,nvICMSUFDest,nvICMSUFRemet,nvBCUFDest,aICMUFDest,nValIpiBene,npFCPUFDest,npICMSUFDest,npICMSInter,npICMSIntP,aObsCont,_cEst,_cUFEM,_cMUNEM)
	cString += "</infNFe>"
EndIf
...
EndIf
	...
	If Len(aInfPr) > 0
		cString += '<infadprod>'+ConvType(aInfPr)+'</infadprod>'
	Else
...
Static Function NfeInfAd(cMsgCli,cMsgFis,aPedido,aExp,cAnfavea,aMotivoCont,aNfSa,aNfVinc,aProd,aDI,aNfVincRur,aRet,cNfRefcup,cSerRefcup,cTipo,nIPIConsig,nSTConsig,lBrinde,cVerAmb,aRefECF,nVicmsDeson,nvFCPUFDest,nvICMSUFDest,nvICMSUFRemet,nvBCUFDest,aICMUFDest,nValIpiBene,npFCPUFDest,npICMSUFDest,npICMSInter,npICMSIntP,aObsCont,_cEst,_cUFEM,_cMUNEM)
