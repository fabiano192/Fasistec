...
#DEFINE MAXVALORC  011                                                // Máximo de caracteres por linha de valores numéricos
...
Local lPergunte	:= .T.
Local lRet		:= .T.
Local _cDir			:= GetNewPar("MX_NFEPDF","C:\DANFE\")
local lVerPerg := .T.
...
If nTipo <> 1
	...
	Else
		...
		If !ExistDir(_cDir)
			If MakeDir(_cDir) <> 0
				MsgAlert("Nao foi possível criar o diretório "+_cDir)
				Return
			Endif
		Endif
	...
	ElseIf ValType( _aPar ) <> "U"
		...
		Conout("Passo A")
		lEnd := .F.
		DANFEProc(@oDanfe, @lEnd, cIDEnt, Nil, Nil, @lExistNFe, lIsLoja)
...
For nZ := 1 To nLenDet
	...
	EndIf
		...
		nMaxCod := MaxCod(oDet[nX]:_Prod:_cProd:TEXT, 60)
...
If nfolha==1
	If lMv_Logod
			oDanfe:SayBitmap(043,001,cLogoD,093,054)
		Else
			oDanfe:SayBitmap(043,001,cLogo,093,054)
	EndIF
Endif
...
EndIf
	...
	If _lJob
		_aBoleto := {2,"",SF2->F2_SERIE,SF2->F2_SERIE,SF2->F2_DOC,SF2->F2_DOC,"","",""}

		U_MAX020(_aBoleto,@oDanfe)
	Endif
