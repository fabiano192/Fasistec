...
User Function PrtNfeSef(cIdEnt, cVal1		, cVal2		, oDanfe,;
	...
	Local lPergunte	:= .T.
	Local lRet		:= .T.
	local _cDir:= getnewPar("MV_YNFEPDF","c:\danfepdf\")
	...
	If nTipo <> 1
		...
		If lPergunte
			...
			//		RptStatus( {|lEnd| U_DanfeProc(@oDanfe,@lEnd,cIdEnt,,,@lExistNfe)},"Imprimindo Danfe...")
		EndIf
				...
				For nX := 1 To nLenNotas
					If !Empty(aXML[nX][2])
						...
						If (!Empty(cAutoriza) .Or. !Empty(cCodAutDPEC) .Or. Alltrim(aXML[nX][8]) $ "2,5") .And. !cCodRetNFE $ RetCodDene()
							...
							_cNota    := aNotas[nX][05]
							_cSerie   := aNotas[nX][04]
							_cFornece := aNotas[nX][06]
							_cLoja    := aNotas[nX][07]
...
Static Function PrtDanfe(oDanfe,oNFE,cCodAutSef,cModalidade,oNfeDPEC,cCodAutDPEC,cDtHrRecCab,dDtReceb,aNota)
	...
	If MV_PAR04 == 1 // ENTRADA
		...
		If Type("oNF:_InfNfe:_infAdic:_infAdFisco:TEXT")<>"U"
			_cAux := oNF:_InfNfe:_infAdic:_infAdFisco:TEXT
			While !Empty(cAux)
				_nPos := AT("MENS-",_cAux)
				If _nPos <= _nTamMen .And. _nPos > 1
					aadd(aMensagem,SubStr(_cAux,1,_nPos-1))
					_cAux := Substr(_cAux,_nPos+5,Len(_cAux))
				Else
					aadd(aMensagem,SubStr(_cAux,1,_nTamMen))
					_cAux := SubStr(_cAux,_nTamMen+1,Len(_cAux))
				Endif
			EndDo
		EndIf

		If Type("oNF:_InfNfe:_infAdic:_infCpl:TEXT")<>"U"
			_cAux := oNF:_InfNfe:_infAdic:_InfCpl:TEXT
		...
		If _cTipoNF $ "B/D"
			SD1->(dbSetOrder(1))
			If SD1->(dbSeek(xFilial("SD1")+ _cNota + _cSerie + _cFornece + _cLoja))
				...
				While SD1->(!Eof()) .And. _cChavSD1 == SD1->D1_DOC + SD1->D1_SERIE + SD1->D1_FORNECE + SD1->D1_LOJA
					...
					If Ascan(_aNfRet,SD1->D1_NFORI) == 0
						AADD(_aNfRet,SD1->D1_NFORI)
						_cNfRet += "/"+SD1->D1_NFORI
					Endif
			...
			_cRet := "REF. A(S) NOTA(S) FISCAL(S) No. "+_cNfRet
			aadd(aMensagem,_cRet)
		Endif
	Else
		aMensagem := {}
		If Type("oNF:_InfNfe:_infAdic:_infAdFisco:TEXT")<>"U"
		...
		//MENSAGENS PERSONALIZADAS - ADICIONADAS NO NFESEFAZ.PRW EM UMA STRING SEPARADA POR #
		If Type("oNF:_InfNfe:_infAdic:_infCpl:TEXT")<>"U"
			cAux2 := oNF:_InfNfe:_infAdic:_InfCpl:TEXT
			aAux := STRTOKARR(cAux2,"#")
			For nX := 1 To Len(aAux)
				cAux := aAux[nX]  //oNF:_InfNfe:_infAdic:_InfCpl:TEXT
				While !Empty(cAux)

					// comentado por sergio em 19/01/10 devido a utilização da variavel personalizada
					// cMens... logo abaixo
					//				aadd(aMensagem,SubStr(cAux,1,105)) Normando (Semar) 25/09/2015 - Alinhar dados na obs da nota
					aadd(aMensagem,SubStr(cAux,1,89))

					//				cAux := SubStr(cAux,106) Normando (Semar) 25/09/2015 - Alinhar dados na obs da nota
					cAux := SubStr(cAux,90)
				EndDo
			Next nX
		EndIf
		...
		If _cTipoNF $ "I/P/C/D"
			SD2->(dbSetOrder(3))
			If SD2->(dbSeek(xFilial("SD2")+ _cNota + _cSerie + _cFornece + _cLoja))
				...
				While SD2->(!Eof()) .And. _cChavSD2 == SD2->D2_DOC + SD2->D2_SERIE + SD2->D2_CLIENTE + SD2->D2_LOJA
					...
					If Ascan(_aNfRet,SD2->D2_NFORI) == 0
						AADD(_aNfRet,SD2->D2_NFORI)
						If !_lPrim
							_lPrim := .t.
							_cNfRet += SD2->D2_SERIORI+"-"+SD2->D2_NFORI
						Else
							_cNfRet += "/"+SD2->D2_SERIORI+"-"+SD2->D2_NFORI
						Endif
					Endif
			...
			If _cTipoNF $ "I/P/C"
				_cRet := "COMPLEMENTO REF.A NOTA FISCAL  No. "+_cNfRet
			Else
				_cRet := "REF. A(S) NOTA(S) FISCAL(S) No. "+_cNfRet
			Endif
	...
	*/
	...
	If nQtd > 0
		If oNF:_INFNFE:_VERSAO:TEXT >= "3.10"
			cQtdTot := "             (Quantidade: "+Alltrim(Transform(nQtd,"@E 999,999,999.99"))+"     Emissao: "+ConvDate(oIdent:_DHEMI:TEXT)+")"
		Else
			cQtdTot := "             (Quantidade: "+Alltrim(Transform(nQtd,"@E 999,999,999.99"))+"     Emissao: "+ConvDate(oIdent:_DEMI:TEXT)+")"
		Endif
	Else
		cQtdTot := ""
	Endif
	...
	oDanfe:Say(006, 002, "RECEBEMOS DE "+NoChar(oEmitente:_xNome:Text,lConverte)+" OS PRODUTOS CONSTANTES DA NOTA FISCAL INDICADA AO LADO" +cQtdTot, oFont07:oFont)
