...
±±ºPrograma  ³ BRI075   ºAutor  ³Alexandro da Silva  º Data ³  24/07/16   º±±
...
User Function BRI075()

	/*
	MZ202_A-fGrvLibBor
	BRI75_B-FGRVREAV
	BRI75_C-VISUALIZA
	BRI75_D-VISUALIZA TODOS
	BRI75_E-BLOQUEIO
	...
	*/
	...
	//_aEmpresas:= {"0201","0202","0203","0205","0206","0207","0208","0209","0210","0211","0212","0213","0214","0215","0216","0401",;
	// 			  "0402","0602","0902","1301","1302","1304","1306","1307","1601",;
	// 			  "5001","5002","5003","5004","5005","5006","5007","5008","5009","5010","5012","5013","5014","5015"}

	_aEmpresas:= {"0402","0602","0902","1301","1302","1304","1306","1307","1601","5001","5002","5003","5004","5005","5006","5007","5008","5009","5010","5012","5013","5014","5015"}
	...
	@ 005, 000 GROUP oGroup3 TO aPObjs[1][3], 175 PROMPT "Empresa + Tipo de Liberacao" OF oDlg COLOR 0, 16777215 PIXEL //OK

	DEFINE DBTREE oDBTree2 FROM 020, 001 TO aPObjs[1][3]- 20, 170 OF oGroup3
	...
	For AX:= 1 To Len(_aEmpresas)
		...
		_cQ += " AND ZAH_EMISSA >= '"+DTOS(MV_PAR02-60)+"'"
	...
	@ 005, 177 GROUP oGroup4 TO aPObjs[1][3], aPObjs[1][4] PROMPT "Marque os Documentos" OF oDlg COLOR 0, 16777215 PIXEL

	@ aPObjs[1][3] - 25 ,aPObjs[1,1]  + 190    BUTTON  "Pesquisar"          SIZE 50,10 ACTION Pesquisar(aWBrowse2) 			     PIXEL
	@ aPObjs[1][3] - 25 ,aPObjs[1][1] + 250    BUTTON  "Visualizar"         SIZE 50,10 ACTION BRI75_C("V",oDBTree2:GetPrompt())   PIXEL
	@ aPObjs[1][3] - 25 ,aPObjs[1][1] + 310    BUTTON  "Liberar"            SIZE 50,10 ACTION BRI75_C("L",oDBTree2:GetPrompt())   PIXEL
	@ aPObjs[1][3] - 25 ,aPObjs[1][1] + 370    BUTTON  "DesMarcar Todos"    SIZE 50,10 ACTION MarkTd(aWBrowse2,.f.)               PIXEL
	@ aPObjs[1][3] - 25 ,aPObjs[1][1] + 430    BUTTON  "Marcar Todos"       SIZE 50,10 ACTION MarkTd(aWBrowse2,.T.)               PIXEL
	@ aPObjs[1][3] - 25 ,aPObjs[1][1] + 490    BUTTON  "Bloquear"           SIZE 50,10 ACTION BRI75_C("B",oDBTree2:GetPrompt())   PIXEL
	@ aPObjs[1][3] - 25 ,aPObjs[1][1] + 550    BUTTON  "Fechar"             SIZE 50,10 ACTION odlg:End()                          PIXEL
...
Static Function fWBrowse2()
	...
	AAdd( aObj2, { 100, 60,.T.,.T. } )
	AAdd( aObj2, { 100,100,.T.,.T.,.T. } )

	aInfo2    := { aSize[1],aSize[2],aSize[3],aSize[4], 3, 3 }
	aPosObj2  := MsObjSize( aInfo2, aObj2,.T. )
	aPosGetD  := { 5, 5, aPosObj2[ 2, 4 ] - 18, aPosObj2[ 2, 3 ] - 8 }
	...
	@ 020, 180 LISTBOX oWBrowse2   Fields HEADER "","","Documento","Tipo Doc","Observação"+Space(20),"Emissao" ,"Valor"  ,"Liberador","Data Liberacao","Filial Original" SIZE aPosObj2[2,3]-180,aPosObj2[2,4]+50 PIXEL ColSizes 50,50
...
Static Function BRI75_C(_cMod,_cPrompt)
...
Static Function BRI75_D(_cMod,_lOutEmp,_nCont)
	...
	If _lOutEmp
		...
		ElseIf _cTipoZAH == "07"
			cModo := "E"
			EmpOpenFile("SC5","SC5",1,.T.,_cEmpresa,@cModo)
			EmpOpenFile("SC6","SC6",1,.T.,_cEmpresa,@cModo)
		ElseIf _cTipoZAH == "02" //Preço Pedido de Vendas
			cModo := "E"
			EmpOpenFile("SC5","SC5",1,.T.,_cEmpresa,@cModo)
			EmpOpenFile("SC6","SC6",1,.T.,_cEmpresa,@cModo)
		Endif
...
//		_cChave  := Substr(SCR->CR_NUM,1,TAMSX3("C6_NUM")[1])
	...
	ElseIf SCR->CR_TIPO == "06" // BORDERO DE PAGAMENTO
		...
		_aCampos := {"EA_PREFIXO","EA_NUM","EA_PARCELA","EA_TIPO","EA_FORNECE","EA_LOJA","E2_EMISSAO","C7_EMISSAO","E2_VENCREA","A2_NOME","E2_VALOR","E2_HIST"}

		For AX:= 1 TO Len(_aCampos)
			dbSelectArea("Sx3")
			dbSetOrder(2)
			If dbSeek(_aCampos[AX])
				If _aCampos[AX] != "C7_EMISSAO"
					AADD(aHeader,{ TRIM(x3_titulo), x3_campo, x3_picture,;
					x3_tamanho, x3_decimal,x3_valid,;
					x3_usado, x3_tipo, x3_arquivo, x3_context } )
				Else
					AADD(aHeader,{ "Emissao Pedido", x3_campo, x3_picture,;
					x3_tamanho, x3_decimal,x3_valid,;
					x3_usado, x3_tipo, x3_arquivo, x3_context } )
				Endif
			Endif
		...
		Private _nPDTEMI   := aScan( aHeader, { |x| Alltrim(x[2])== "E2_EMISSAO" } )
		Private _nPDTEMPC  := aScan( aHeader, { |x| Alltrim(x[2])== "C7_EMISSAO" } )
		...
		SEA->(dbSetOrder(2))
		If SEA->(dbSeek(_cFilNew + _cBordero + "P" ))

			_cChavSEA := SEA->EA_NUMBOR + SEA->EA_CART

			While SEA->(!Eof()) .And. _cChavSEA == SEA->EA_NUMBOR + SEA->EA_CART
				...
				SE2->(dbSetOrder(1))
				SE2->(dbSeek(SEA->EA_FILORIG + SEA->EA_PREFIXO + SEA->EA_NUM + SEA->EA_PARCELA +SEA->EA_TIPO + SEA->EA_FORNECE + SEA->EA_LOJA))
				...
				aCols[Len(aCols),_nPDTEMI]   := SE2->E2_EMISSAO
				aCols[Len(aCols),_nPDTEMPC]  := _dDtPedido
	...
	ElseIf SCR->CR_TIPO == "07" // PEDIDO DE VENDA
		...
		_aCampos := {"C6_ITEM","C6_PRODUTO","C6_DESCRI","C6_QTDVEN","C6_UM","C6_PRCVEN","C6_VALOR","C6_TES","C6_CF"}
		...
		Private _nPDESCRI  := aScan( aHeader, { |x| Alltrim(x[2])== "C6_DESCRI"  } )
		Private _nPQUANT   := aScan( aHeader, { |x| Alltrim(x[2])== "C6_QTDVEN"  } )
		Private _nPUM      := aScan( aHeader, { |x| Alltrim(x[2])== "C6_UM"      } )
		Private _nPPRCVEN  := aScan( aHeader, { |x| Alltrim(x[2])== "C6_PRCVEN"  } )
		Private _nPVALOR   := aScan( aHeader, { |x| Alltrim(x[2])== "C6_VALOR"   } )
		Private _nPTES     := aScan( aHeader, { |x| Alltrim(x[2])== "C6_TES"     } )
		Private _nPCF      := aScan( aHeader, { |x| Alltrim(x[2])== "C6_CF"      } )

		aCols     := {}
		_cPedido  := Substr(SCR->CR_NUM,1,TAMSX3("C6_NUM")[1])
		_cItemPV  := Substr(SCR->CR_NUM,7,TAMSX3("C6_ITEM")[1])
		//SC9->C9_PEDIDO + SC9->C9_ITEM + SC9->C9_PDOC
		_cFilNew := SCR->CR_FILIAL
		...
		If SC9->(dbSeek(_cNewFil + Left(SCR->CR_NUM,15)))
			...
			SC6->(dbSetOrder(1))
			SC6->(dbSeek(SCR->CR_FILIAL + SC9->C9_PEDIDO))
			...
			aCols[Len(aCols),_NPITEM]    := SC9->C9_ITEM
			aCols[Len(aCols),_NPPROD]    := SC9->C9_PRODUTO
			aCols[Len(aCols),_NPDESCRI]  := SB1->B1_DESC
			aCols[Len(aCols),_NPQUANT]   := SC9->C9_QTDLIB
			aCols[Len(aCols),_NPUM]      := SB1->B1_UM
			aCols[Len(aCols),_NPPRCVEN]  := SC9->C9_PRCVEN
			aCols[Len(aCols),_NPVALOR]   := SC9->C9_QTDLIB * SC9->C9_PRCVEN
			aCols[Len(aCols),_NPTES]     := SC6->C6_TES
			aCols[Len(aCols),_NPCF]      := SC6->C6_CF
			aCols[Len(aCols),Len(_aCampos)+1]:=.F.
	...
	ElseIf SCR->CR_TIPO == "02" // PRECO PEDIDO DE VENDA
		...
		_aCampos := {"C6_ITEM","C6_PRODUTO","C6_DESCRI","C6_QTDVEN","C6_UM",'Z2_PRECO',"C6_PRCVEN","C6_VALOR","C6_TES","C6_CF"}

		For AX:= 1 TO Len(_aCampos)
			SX3->(dbSetOrder(2))
			If SX3->(MsSeek(_aCampos[AX]))
				If _aCampos[AX] = 'C6_PRCVEN'
					_cTit := 'Preco Pedido'
				ElseIf _aCampos[AX] = 'Z2_PRECO'
					_cTit := 'Preco Tabela'
				Else
					_cTit := TRIM(SX3->X3_TITULO)
				Endif
		...
		Private _nPTES     := aScan( aHeader, { |x| Alltrim(x[2])== "C6_TES"     } )
		Private _nPCF      := aScan( aHeader, { |x| Alltrim(x[2])== "C6_CF"      } )
		Private _nPPRTAB   := aScan( aHeader, { |x| Alltrim(x[2])== "Z2_PRECO"   } )
...
//		_cNewFil  := "z"+substr(_cFilNew,2,1)
		...
		SC6->(dbSetOrder(1))
		If SC6->(MsSeek(_cNewFil + Left(SCR->CR_NUM,8)))
			...
			SZ2->(dbSetOrder(4))
			SZ2->(msSeek(xFilial("SZ2")+SC6->C6_CLI  + SC6->C6_LOJA + SC6->C6_PRODUTO ))

			AADD(aCols,Array(Len(_aCampos)+1))

			aCols[Len(aCols),_NPITEM]    := SC6->C6_ITEM
			aCols[Len(aCols),_NPPROD]    := SC6->C6_PRODUTO
			aCols[Len(aCols),_NPDESCRI]  := SB1->B1_DESC
			aCols[Len(aCols),_NPQUANT]   := SC6->C6_QTDVEN
			aCols[Len(aCols),_NPUM]      := SB1->B1_UM
			aCols[Len(aCols),_NPPRCVEN]  := SC6->C6_PRCVEN
			aCols[Len(aCols),_NPVALOR]   := SC6->C6_QTDVEN * SC6->C6_PRCVEN
			aCols[Len(aCols),_NPTES]     := SC6->C6_TES
			aCols[Len(aCols),_NPCF]      := SC6->C6_CF
			aCols[Len(aCols),_nPPRTAB]   := SZ2->Z2_PRECO
			aCols[Len(aCols),Len(_aCampos)+1]:=.F.
...
Static Function MZ56_06(cTitulo,cAlias2   ,cLinOk,cTudOk,_nOpcE,_nOpcG,cFieldOk,_lOutEmp,_cMod,_nCont)
	...
	If SCR->CR_TIPO == "PC"
		...
		*/
		@ 2.6,002 Say "Pedido: "
		@ 2.6,005 MSGET oPedido VAR _cPedido              When .F.     PICTURE "@!" SIZE 30,10

		@ 2.6,010 Say "Fornecedor: "
		@ 2.6,014 MSGET oFornece VAR _cFORNECE            When .F.     PICTURE "@!" SIZE 30,10

		@ 2.6,020 Say "Loja: "
		@ 2.6,023 MSGET oLoja    VAR _cLoja               When .F.     PICTURE "99" SIZE 20,10

		@ 2.6,027 Say "Nome Fornecedor: "
		@ 2.6,034 MSGET oNomFor  VAR _cNOMFOR             When .F.     SIZE 150,10

		@ 2.6,054 Say "Emissao: "
		@ 2.6,057 MSGET oEmissao VAR _dEmissao            When .F.     SIZE 30,10

		@ 3.8,002 Say "Nome do Usuario: "
		@ 3.8,010 MSGET oUsuario VAR Alltrim(_cNomUser)   When .F.     SIZE 70,10

		@ 3.8,020 Say "Mercadoria: "
		@ 3.8,025 MSGET oMercad  VAR _nMERCAD             When .F.     PICTURE "@E 999,999.99" SIZE 50,10

		@ 3.8,032 Say "Despesas/Frete: "
		@ 3.8,038 MSGET oDespesa VAR _nOUTROS             When .F.     PICTURE "@E 999,999.99" SIZE 50,10

		@ 3.8,045 Say "Total Pedido:"
	...
	ElseIf SCR->CR_TIPO == "05"
		@ 2.8,002 Say "Cliente: "
		@ 2.8,005 MSGET oCliente VAR _cCliente         When .F.     PICTURE "@!" SIZE 30,10

		@ 2.8,010 Say "Loja: "
		@ 2.8,014 MSGET oLoja    VAR _cLoja            When .F.     PICTURE "99" SIZE 30,10

		@ 2.8,020 Say "Nome Cliente: "
		@ 2.8,025 MSGET oNomCli  VAR _cNomCli          When .F.     SIZE 120,10

		@ 2.8,041 Say "Saldo Limite: "
		@ 2.8,045 MSGET oSdoLim  VAR _nSdoLim          When .F.     PICTURE "@E 999,999,999.99" SIZE 50,10

		@ 2.8,053 Say "Saldo Titulo: "
		@ 2.8,057 MSGET oSdoTit  VAR _nSdoTit          When .F.     PICTURE "@E 999,999,999.99" SIZE 50,10
	ElseIf SCR->CR_TIPO == "06"
		@ 2.8,002 Say "Bordero: "
		@ 2.8,005 MSGET oBordero VAR _cBordero         When .F.     PICTURE "@!" SIZE 30,10
		@ 2.8,023 Say "Total do Bordero: "
		@ 2.8,030 MSGET oTotBor  VAR SCR->CR_TOTAL     When .F.     PICTURE "@E 999,999,999.99" SIZE 50,10
//	ElseIf SCR->CR_TIPO == "07"
	...
	ElseIf SCR->CR_TIPO == "NF"
		@ 2.6,002 Say "N.Fiscal:
		@ 2.6,005 MSGET oPedido VAR _cNota             When .F.     PICTURE "@!" SIZE 30,10
		...
		@ 3.8,030 Say "TOTAL DA NOTA FISCAL --> "
		@ 3.8,038 MSGET oTotPed  VAR _nTOTPED          When .F.     PICTURE "@E 999,999,999.99" SIZE 50,10

		_nTop := aPosObj[2,1] + 7

	Else
		@ 2.8,001 Say "Prefixo: "
		@ 2.8,004 MSGET oPrefixo  VAR _cPrefixo   When .F.  PICTURE "@!"  SIZE 30,10

		@ 2.8,010 Say "Numero: "
		@ 2.8,013 MSGET oNumero   VAR _cNum       When .F.  PICTURE "@!"  SIZE 50,10

		@ 2.8,022 Say "Parcela: "
		@ 2.8,025 MSGET oParcela  VAR _cParcela   When .F.  PICTURE "@!"  SIZE 10,10

		@ 2.8,029 Say "Cliente: "
		@ 2.8,032 MSGET oCliente  VAR _cCliente   When .F.  PICTURE "@!"  SIZE 30,10

		@ 2.8,039 Say "Loja: "
		@ 2.8,041 MSGET oLoja     VAR _cLoja      When .F.  PICTURE "@!"  SIZE 10,10
...
User Function BRI75_B(_cEmp,_cFil,_cAprovador,_cTipoZAH,_cNumZAH,_cUserZAH,_cMod,CA097USER)
...
User Function BRI75_E(_cEmp    ,_cFil,_cAprovador,_cTipoZAH,_cNumZAH,_cUserZAH,_nCont,_cMod ,_lOutEmp,oDlg)

	_cFilNew := SCR->CR_FILIAL

	If _lOutEmp                                  //  1         2               3          4          5        6       7
		StartJob("U_BRI75_B", GetEnvServer(), .T., _cEmp     , _cFilNew       ,_cAprovador,_cTipoZAH,_cNumZAH,_cUserZAH,_cMod,CA097USER)
	Else       //   1        2       3       4    5
		U_BRI072(_cEmpresa, _cFilNew, _cAprovador, _cMod, CA097USER )
	Endif
