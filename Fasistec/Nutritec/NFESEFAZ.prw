...
If cTipo == "1"
	...
	For nCountNF := 1 To Len(aCupRefLoj)
		...
		//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
					...
					If Ascan(_aPedido,AllTrim(SC5->C5_NUM)) = 0
						AADD(_aPedido,AllTrim(SC5->C5_NUM))

						If !Empty(SC5->C5_NPEDCLI)
							cMensCli += "MENS- PEDIDO CLIENTE No : "+ AllTrim(SC5->C5_NPEDCLI)
						EndIf

						cMensCli += "MENS- PEDIDO NUTRITEC No : "+ AllTrim(SC5->C5_NUM)
					Endif
