...
// ShowHelpDlg("XML_TES", {'Campo "A5_YTES" não existe na Base de Dados!'},2,{'Solicite a criação do mesmo.'},2)
// Return(Nil)
// Endif

	Private lBtnFiltro := .F.
	Private lUsacolab  := .F.
	Private _lVldFil   := VldFil()
...
Função de montagem das perguntas e condição para efetuar o filtro
...
User Function ASBtLegenda()
	...
	AADD(aLegenda, {"BR_AMARELO" ,"Manifestação em processamento"})
					...
					For nX := 1 To Len(aDocs)
						If Type(aDocs[nX]:CCHAVE) <> "U" .and. Type(aDocs[nX]:CSITCONF) <> "U"
							...
							If !cCodEvento $ "411500|411501|411502|411503"
								...
								_cQry := " SELECT C00_CHVNFE FROM "+RetSqlName("C00")+" C00 " +CRLF
								_cQry += " WHERE C00.D_E_L_E_T_ = '' " +CRLF
								_cQry += " AND C00_CHVNFE = '"+Alltrim(cChave)+"' " +CRLF

								TcQuery _cQry New Alias "TCHAVE"

								If Contar("TCHAVE","!EOF()") = 0
...
User Function ASManif(cAlias, nReg, nOpc,cMarca, lInverte)
	...
	If ReadyTss()
		...
		If Len(aListBox) > 0
			...
			EndIF
		Else
			Aviso("Envio Manifesto",STR0440,{"OK"},3)//"Selecione uma ou mais chaves pendentes de manifestação"
		EndIf
...
//Váriável que define se vai ser a exportação direta ou irá montar browse para marcar - usado apenas para TOTVS Colaboração
	...
	If ReadyTss()
		...
		If &(_cBloco)
			...
			If !_lOk_
				MsgInfo("Nota fiscal sem retorno da Manifestação junto à SEFAZ!")
				Return(Nil)
			Endif
				...
				Else
					...
					If _cTipo = "D"
						...
						If SA1->(msSeek(xFilial("SA1")+_oNFE:_INFNFE:_EMIT:_CNPJ:TEXT))
							...
							_cNome := SA1->A1_NOME
						Else
							// ShowHelpDlg("XML_ZA1", {'Fornecedor não encontrado!'},2,{'Cadastre o Fornecedor.'},2)
							// Return(.F.)
						Endif
					Else
						SA2->(dbSetOrder(3))
			...
			If Type("oWS:oWsNfemonitorLoteEventoResult:OWSNfeMonitorEvento") <> "U"
				...
				For nX:=1 To Len(aMonitor)
					AADD( aListBox, { If(aMonitor[nX]:nStatus <> 6 .And. aMonitor[nX]:nStatus <> 7 ,oNo,oOk),;
						IIf(aMonitor[nX]:nProtocolo <> 0 ,Alltrim(Str(aMonitor[nX]:nProtocolo)),""),;
						aMonitor[nX]:cId_Evento,;
						Alltrim(Str(aMonitor[nX]:nAmbiente)),;
						Alltrim(Str(aMonitor[nX]:nStatus)),;
						IIf(!Empty(aMonitor[nX]:cCMotEven),Alltrim(aMonitor[nX]:cCMotEven),Alltrim(aMonitor[nX]:cMensagem)),;
	...
	For _nTen_ := 1 To 10
		...
		For _nList := 1 to Len(aListChv)
			If aListChv[_nList][5] $ "6|7"
				_lOk_ := .T. //Variável que define se houve retorno do SEFAZ.
				_nList := Len(aListChv)
				_nTen_ := 10
			Else
				Sleep(3000)
...
User Function ASVldFilial()
	...
	If ZA4->(MsSeek(xFilial("ZA4")+cFilAnt))
		...
		ZA4->(dbSetOrder(1))
		If ZA4->(MsSeek(xFilial("ZA4")+_cGrp))
	...
	Else
		ShowHelpDlg("AS_MANIF_1", {'Filial não cadastrada para Validação!'},2,{'Não se aplica.'},2)
		RestArea(_AreaSM0)
		Return(Nil)
	Endif
	...
	*/
	...
	_cQry := " SELECT C00_CHVNFE FROM "+RetSqlName("C00")+" C00 " +CRLF
	_cQry += " WHERE C00.D_E_L_E_T_ = '' " +CRLF
	_cQry += " AND C00.C00_FILIAL = '"+xFilial("C00")+"' AND C00_YSTATU = 'FI' AND C00.C00_OK = '"+cMarkDlg+"' "+cCondQry+" "
	...
	_nRadio := aScan(_aRadio,{|x| Left(x,2) = cFilAnt})
	...
	TButton():New( 050, 100, "Cancelar" ,_oOdlgFil,{||(_nOpc := 0,_oOdlgFil:End())},30,10,,,.F.,.T.,.F.,,.F.,,,.F. )
	...
	If _nOpc = 1
		...
		While TC00->(!Eof())
			...
			C00->(DbsetOrder(1))
			If C00->( DbSeek( xFilial("C00") + TC00->C00_CHVNFE) )
				C00->(RecLock("C00",.F.))
