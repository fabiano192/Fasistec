...
*---------------------------------------------------------------------------------------
	...
	// Local _lVldPes	:= SuperGetMv("BRI_VLDPES",,.F.)
	Local _lVldPes	:= If(Left(UPPER(Alltrim(ComputerName())),3) = "IBA",.T.,.F.)
	Local _nPercTo	:= SuperGetMv("BRI_TOLPES",,0)
	...
	If _lVldPes .And. SZ1->(FieldPos("Z1_CODPESO")) > 0
		_wAreaSZ1 := SZ1->(GetArea())
		SZ1->(dbSetOrder(1))
		If SZ1->(MsSeek(xFilial("SZ1")+_cPlaca))
			If !Empty(SZ1->Z1_CODPESO)
				ZF5->(dbSetorder(1))
				IF ZF5->(MsSeek(xFilial("ZF5")+SZ1->Z1_CODPESO))
					_nPercTo := If(ZF5->ZF5_PERCTO > 0, ZF5->ZF5_PERCTO,_nPercTo)
					_nPesBrt := ZF5->ZF5_PEBRTO + (ZF5->ZF5_PEBRTO * _nPercTo / 100)
					If _nPesBrt < _nPBruto
						ShowHelpDlg("PDFATA010_1", {'Peso informado maior que o cadastro de Caminhão.','',;
						'Veículo cadastro: '+Alltrim(ZF5->ZF5_VEICUL),'Peso Máximo: '+Alltrim(Str(ZF5->ZF5_PEBRTO,10,2))},4,;
						{'Valide o conteúdo do campo "Cod. Peso" no cadastro de Caminhão.'},1)
						Return(.F.)
					Endif
				Endif
			Endif
