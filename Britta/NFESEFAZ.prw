    ...
    If cTipo == "1"
        ...
        For nCountNF := 1 To Len(aCupRefLoj)
            ...
            If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
                ...
                ElseIf FunName() == "SPEDNFSE"
                    ...
                    aadd(aDest,SA1->A1_EMAILNF)  //aadd(aDest,SA1->A1_EMAIL)
                    _cEst := SA1->A1_EST
				...
				//ALTERADO ABAIXO - FORÇAMOS A PLACA (DADOS DO VEICULO)
                            If !Empty(SF2->F2_PLACA)
					aadd(aVeiculo,SF2->F2_PLACA)
					//aadd(aVeiculo,"SP")// ALTERADO DEVIDO AO USO DE TRANSPORTADORA EM VARIOS ESTADOS. - CARLOS  INFORMATICA POLIMIXD
					//aadd(aVeiculo,SF2->F2_EST)
					aadd(aVeiculo,SA4->A4_EST) // ALTERADO PARA SAIR UF DO TRANSPORTADOR
					//aadd(aVeiculo,"")//RNTC 
					aadd(aVeiculo,Alltrim(SA4->A4_YANTT))//ALTERADO POR ALEXANDRO EM 24/09/15

                                If SM0->M0_CODIGO $ "01/02/08"
						dbSelectArea("SZ1")
						dbSetOrder(1)
                                    If dbSeek(xFilial("SZ1")+SF2->F2_PLACA)
							_cCodMot   := SZ1->Z1_CODMOT
							_cNomeMot  := Posicione("SA3",1,xFilial("SA3")+_cCodMot,"A3_NOME")
                                    Endif
                                Endif
                            Endif
				...
				//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
					...
					cAliasSE1 := GetNextAlias()
						...
						//utiliza a funcao SpedNatOper ( SPEDXFUN ) que possui o tratamento para a natureza da operacao/prestacao
                                    if FindFunction( "SpedNatOper" ) .And. SFT->( MsSeek( xFilial( "SFT" ) +cChave) )
                                        If !Alltrim(SpedNatOper( nil , lNatOper , "SFT" , "SF4" , .T. )[ 2 ])$cNatOper
                                            If	Empty(cNatOper).and. !SF4->F4_CODIGO $ "743/744"
									cNatOper := SpedNatOper( nil , lNatOper , "SFT" , "SF4" , .T. )[ 2 ]  
                                            elseif 	Empty(cNatOper).and. SF4->F4_CODIGO $ "743/744"
						...
						_cPedCli := ""
						_cLinCli := ""
                                    If	SC5->(FieldPos("C5_YPEDCLI")) <> 0 .AND. SC6->(FieldPos("C6_YLINCLI")) <> 0
							_cPedCli := SC5->C5_YPEDCLI
							_cLinCli := SC6->C6_YLINCLI
                                    Endif
						...
						u_NfeDadosAdic( @cMensCli, SC5->C5_NUM, cAliasSD2 )
						...
						*/
                                    ...
                                    If cEmpant $ "12/32" .Or. (cEmpAnt == "50" .And. cFilAnt == "04")
                                        //_nPrcVen := IIF(!(cAliasSD2)->D2_TIPO$"IP", IIf(!(cAliasSD2)->D2_TIPO $ "D", (cAliasSD2)->D2_PRCVEN,0), 0) //16
                                        _nPrcVen := IIF(!(cAliasSD2)->D2_TIPO$"IP", (cAliasSD2)->D2_PRCVEN, 0) //16
                                    Else
                                        _nPrcVen := IIF(!(cAliasSD2)->D2_TIPO$"IP", IIf(!(cAliasSD2)->D2_TIPO $ "D", ( (cAliasSD2)->D2_TOTAL + nDesconto + (cAliasSD2)->D2_DESCZFR) / (cAliasSD2)->D2_QUANT, (cAliasSD2)->D2_PRCVEN), 0)
                                    Endif
						...
						SB1->B1_UM,;
						(cAliasSD2)->D2_QUANT,;
						IIF(!(cAliasSD2)->D2_TIPO$"IP",Iif(!(cAliasSD2)->D2_TIPO$"D",(cAliasSD2)->D2_TOTAL + nDesconto + (cAliasSD2)->D2_DESCZFR,(cAliasSD2)->D2_TOTAL + nDesconto),0),;  //10
						...
						*/
                                    ...
                                    If cEmpAnt $ "04/11/13/14/31/33/34/50"   //Incluso as empresas 31/34 - Alison  //Incluso a empresa 04 e 50 - Bruno
                                        If SF2->F2_CLIENTE $ GETMV("BRI_CLIVAL")
                                            aPedCom   	:= {}
                                            aadd(aPedCom,{_cPedCli,_cLinCli})
                                        Else
                                            aadd(aPedCom,{})
                                        Endif
                                    Else
		...
		dbSetOrder(1)
			...
			//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ			
				...
				xBASE_PIS   :=SF1->F1_BASIMP2
				xBASE_COF   :=SF1->F1_BASIMP3
				xVAL_PIS    :=SF1->F1_VALIMP2
				xVAL_COF    :=SF1->F1_VALIMP3
				_cEst       :=SF1->F1_EST
					...
					*/
					...
					/*Tratamento para NF DE AJUSTE chamado THYZ13 -  PORTARIA N° 163/2007 Artigo 18-B-2 item 4a da SEFAZ-MT */
                                                                        ...
                                                                        If ( nScan2 == 0 )
                                                                            ...
                                                                            //If (cAliasSD1)->D1_VALIMP2 > 0    /// PIS
                                                                            If SF1->F1_VALIMP2 > 0    /// PIS
                                                                            ...
                                                                            //If (cAliasSD1)->D1_VALIMP3 > 0    /// COFINS
                                                                            If SF1->F1_VALIMP3 > 0    /// COFINS
                                                                        ...
                                                                        If SFT->FT_DESCZFR>0
                                                                            ...
                                                                            While !CD2->(Eof()) .And. xFilial("CD2") == CD2->CD2_FILIAL .And.;
                                                                                ...
                                                                                Case AllTrim(CD2->CD2_IMP) == "ICM"
                                                                                    ...
                                                                                    CD2->CD2_BC,;
                                                                                        Iif(cVerAmb == "4.00" .and. FindFunction("xFisRetFCP"), xFisRetFCP('4.0','CD2','CD2_ALIQ'), Iif(CD2->CD2_BC>0,CD2->CD2_ALIQ,0)),;
    ...
    ElseIf _cEst == "EX"
        ...
        If cVerAmb >= "3.10"
			...
			*/ 
            ...
            cString += NfeTag('<CNPJ>'+Space(14))
            cString += NfeTag('<UfTerceiro>'+'SP')
        EndIf
...
Static Function NfeInfAd(cMsgCli,cMsgFis,aPedido,aExp,cAnfavea,aMotivoCont,aNfSa,aNfVinc,aProd,aDI,aNfVincRur,aRet,cNfRefcup,cSerRefcup,cTipo,nIPIConsig,nSTConsig,lBrinde,cVerAmb,aRefECF,nVicmsDeson,nvFCPUFDest,nvICMSUFDest,nvICMSUFRemet,nvBCUFDest,aICMUFDest,nValIpiBene,npFCPUFDest,npICMSUFDest,npICMSInter,npICMSIntP,aObsCont)
	...
	*/
    ...
    If _cEst = 'EX' .And. cTipo = "1"
        cString += '<exporta>'
        //	cString += '<UFEmbarq>'+_cUFEM+ '</UFEmbarq>'
        //	cString += '<locembarq>'+_cMUNEM+ '</locembarq>'
