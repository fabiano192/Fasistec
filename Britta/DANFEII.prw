...
User Function PrtNfeSef(cIdEnt, cVal1		, cVal2		, oDanfe,;
	...
	If nTipo <> 1
		...
		If ( oDanfe:lInJob )
			...
			DanfeProc(@oDanfe,.F.,cIdEnt,cVal1,cVal2,@lExistNfe,lIsLoja,nTipo)
		Else

			If lPergunte .And. ValType( _aPar ) = "U"
				RptStatus({|lEnd| DANFEProc(@oDanfe, @lEnd, cIDEnt, Nil, Nil, @lExistNFe, lIsLoja, nTipo)},"Imprimindo Danfe...")
			ElseIf ValType( _aPar ) <> "U"
				lEnd := .F.
				DANFEProc(@oDanfe, @lEnd, cIDEnt, Nil, Nil, @lExistNFe, lIsLoja, nTipo)
			EndIf
		EndIf
...
Static Function PrtDanfe(oDanfe,oNFE,cCodAutSef,cModalidade,oNfeDPEC,cCodAutDPEC,cDtHrRecCab,dDtReceb,aNota)
	...
	*/
	...
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	oDanfe:Box(050,000,137,250)
	oDanfe:Say(058,098, "Identificação do emitente",oFont08N:oFont)
	nLinCalc	:=	070
	cStrAux		:=	AllTrim(NoChar(oEmitente:_xNome:Text,lConverte))
	nForTo		:=	Len(cStrAux)/41
	nForTo		+=	Iif(nForTo>Round(nForTo,0),Round(nForTo,0)+1-nForTo,nForTo)
	For nX := 1 To nForTo
		oDanfe:Say(nLinCalc,098,SubStr(cStrAux,Iif(nX==1,1,((nX-1)*41)+1),41), oFont08N:oFont )
		nLinCalc+=10
	Next nX
	...
	*/
	...
	//oDanfe:Box(050,350,102,603)   /// OK BLOCO CODIGO DE BARRAS

	oDanfe:Box(050,248,137,351)
	oDanfe:Say(063,275, "DANFE",oFont16N:oFont)
	oDanfe:Say(075,258, "DOCUMENTO AUXILIAR DA",oFont07:oFont)
	oDanfe:Say(083,258, "NOTA FISCAL ELETRÔNICA",oFont07:oFont)
	oDanfe:Say(091,266, "0-ENTRADA",oFont08:oFont)
	oDanfe:Say(099,266, "1-SAÍDA"  ,oFont08:oFont)
	oDanfe:Box(088,315,103,325)
	oDanfe:Say(097,318, oIdent:_TpNf:Text,oFont08N:oFont)
	oDanfe:Say(110,255,"N. "+StrZero(Val(oIdent:_NNf:Text),9),oFont10N:oFont)
	...
	//³Logotipo                                     ³
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	cLogo := GetSrvProfString("Startpath","") + "logonfe.BMP"
	If lMv_Logod
	...
	If nfolha==1
		If lMv_Logod
			oDanfe:SayBitmap(053,005,cLogoD,080,080)
		Else
			oDanfe:SayBitmap(053,005,cLogo,080,080)
		EndIF
	Endif
	...
	oDanfe:Box(050,350,102,603)   /// OK BLOCO CODIGO DE BARRAS
	oDanfe:Box(085,350,109,603)   /// OK BLOCO CHAVE DE ACESSO
	oDanfe:Say(104,355,TransForm(SubStr(oNF:_InfNfe:_ID:Text,4),"@r 9999 9999 9999 9999 9999 9999 9999 9999 9999 9999 9999"),oFont12N:oFont)
	oDanfe:Box(109,350,137,603)
	...
	If nFolha == 1
		...
		oDanfe:Say(094,355,"CHAVE DE ACESSO DA NF-E",oFont10N:oFont)
		nFontSize := 28
		oDanfe:Code128C(081,370,SubStr(oNF:_InfNfe:_ID:Text,4), nFontSize )
	EndIf
