...
oSetup, cFilePrint	, lIsLoja	, nTipo,_x,_aPar	) //Inserido a variável _x, pois foi acrescentado mais um parâmetro na função  pela TOTVS.
// oSetup, cFilePrint	, lIsLoja	, nTipo,_aPar	)
...
Private _cCLIFOR := Space(06)
Private _cLoja   := Space(02)

Private _lJob		:= .F.
Private _bPerg		:= 'Pergunte("NFSIGW",.T.)'
...
if nTipo<>1
	...
	If ValType( _aPar ) <> "U"
		...
		oDanfe:lServer	:= .T.
		oDanfe:linjob	:= .T.
		oDanfe:cPathPDF:="\WORKFLOW\RELATORIOS\" // diretorio no servidor onde sera gerado o arquivo. pdf caso seja diferente de cLocal
	...
	Else
		...
		If ( !oDanfe:lInJob )
			...
			ElseIf oSetup:GetProperty(PD_PRINTTYPE) == IMP_PDF
				...
				xPathPDF        := oSetup:aOptions[PD_VALUETYPE]
				lPDF := .T.
			Endif
		Endif
	...
	If ( oDanfe:lInJob )
		...
		/* Ricardo 22/07/2018 - Tratamento da impressão automática
		If lPergunte
		...
		*/
		...
		If ValType( _aPar ) <> "U"
			lEnd := .F.
			DanfeProc(@oDanfe,@lEnd,cIdEnt,cVal1,cVal2,@lExistNfe)
		Else
			Conout("Passo B")
			RptStatus({|lEnd| DanfeProc(@oDanfe,@lEnd,cIdEnt,cVal1,cVal2,@lExistNfe)},"Imprimindo Danfe...")
		EndIf
	endif	
	...
	*/
	...
	If lExistNfe .And. ValType( _aPar ) = "U"
		oDanfe:Preview()//Visualiza antes de imprimir
	Else
		If !_lJob
			Aviso("DANFE","Nenhuma NF-e a ser impressa nos parametros utilizados.",{"OK"},3)
...
oDanfe:lInJob:=!(Empty(cVAL2))

lProc	:= If(!_lJob,Pergunte("NFSIGW", !(oDanfe:lInJob)),_lJob)

IF oDanfe:lInJob
	MV_PAR01 := cVal2   //nota de
	MV_PAR02 := cVal2    //nota ate
	MV_PAR03 := cVAl1   //serie
	MV_PAR04 := 2
	MV_PAR06 := 2
	lProc:=.T.
ENDIF
		...
		While !Eof() .And. xFilial("SF3") == (cAliasSF3)->F3_FILIAL .And.;
			...
			If  Empty((cAliasSF3)->F3_DTCANC) .And. &cWhere //.And. AModNot((cAliasSF3)->F3_ESPECIE)=="55"
				...
				If (SubStr((cAliasSF3)->F3_CFO,1,1)>="5" .Or. (cAliasSF3)->F3_FORMUL=="S") .And. aScan(aNotas,{|x| x[4]+x[5]+x[6]+x[7]==(cAliasSF3)->F3_SERIE+(cAliasSF3)->F3_NFISCAL+(cAliasSF3)->F3_CLIEFOR+(cAliasSF3)->F3_LOJA})==0
					...
					_cCLIFOR := (cAliasSF3)->F3_CLIEFOR
					_cLOJA   := (cAliasSF3)->F3_LOJA
					_cCodCli := (cAliasSF3)->F3_CLIEFOR
					_cLojaCli:= (cAliasSF3)->F3_LOJA
					_cNota	 := (cAliasSF3)->F3_NFISCAL
					_cSerie  := (cAliasSF3)->F3_SERIE
					_dData   := (cAliasSF3)->F3_ENTRADA
				EndIf
			EndIf
				...
				ElseIf MV_PAR04==2
					...
					Else
						cCampos += "%SF2.F2_FILIAL FILIAL, SF2.F2_DOC DOC, SF2.F2_SERIE SERIE%"
   	 				     // Ricardo 21/08/2019 - incompativel com TJob
	  				     // cSerie := Padr(MV_PAR03,TamSx3("F2_SERIE")[1])
						cSerie := Padr(MV_PAR03,len(sf2->f2_serie))
						...
						If ( !Empty(cAutoriza) .Or. !Empty(cCodAutDPEC) .Or. Alltrim(aXML[nX][8]) $ "2,5" ) .And. !cCodRetNFE $ RetCodDene()
							...
							ElseIf aNotas[nX][02]=="S" .And. MV_PAR04==2 .And. (oNfe:_NFE:_INFNFE:_IDE:_TPNF:TEXT=="1")
								...
								If MsSeek(xFilial("SF2")+PADR(aNotas[nX][05],TAMSX3("F2_DOC")[1])+aNotas[nX][04]) .And. Alltrim(aXML[nX][8])$"1,3,4,6,7" .Or. ( Alltrim(aXML[nX][8]) $ "2,5"  .And. !Empty(cAutoriza) )
									...
									_cLojaCli := SF2->F2_LOJA
									
									// Ricardo 21/08/2019 - Para envio automático, porém falhou a atualização da chave no uso normal
									RecLock("SF2")
									If !SF2->F2_FIMP$"D"
										SF2->F2_FIMP := "S"
									EndIf
									...
									/*
									If !SF2->F2_FIMP$"D"
									    _cSql:="Update "+RetSqlName("SF2")+" set F2_FIMP='S' where R_E_C_N_O_="+alltrim(str(sf2->(recno())))
										_nReturn:=TcSqlExec(_cSql)
									EndIf
									If SF2->(FieldPos("F2_HAUTNFE")) > 0 .and. SF2->(FieldPos("F2_DAUTNFE")) > 0 //grava a data e hota de autorização da NFe
										_HAUTNFE := IIF(!Empty(aXML[nX][6]),SUBSTR(aXML[nX][6],1,5),"")
										_DAUTNFE	:= IIF(!Empty(aXML[nX][7]),aXML[nX][7],SToD("  /  /    "))
			     
                                        _cSql:="Update "+RetSqlName("SF2")+" set F2_HAUTNFE='"+_HAUTNFE+"',F2_DAUTNFE='"+dtos(_DAUTNFE)+"' where R_E_C_N_O_="+alltrim(str(sf2->(recno())))
										_nReturn:=TcSqlExec(_cSql)                                       
									...
									If SX2->(dbSeek("ZA8"))
										ZA8->(dbSetOrder(1))
										If ZA8->(!dbSeek(xFilial("ZA8")+ cEmpAnt + cFilAnt + SF2->F2_DOC + SF2->F2_SERIE + SF2->F2_CLIENTE + SF2->F2_LOJA + SF2->F2_TIPO ))
											ZA8->(RecLock("ZA8",.T.))
											ZA8->ZA8_FILIAL := xFilial("ZA8")
											ZA8->ZA8_CODEMP := cEmpAnt
											ZA8->ZA8_CODFIL := cFilAnt
											ZA8->ZA8_NOTA   := SF2->F2_DOC
											ZA8->ZA8_SERIE  := SF2->F2_SERIE
											ZA8->ZA8_CLIENT := SF2->F2_CLIENTE
											ZA8->ZA8_LOJA   := SF2->F2_LOJA
											ZA8->ZA8_TIPO   := SF2->F2_TIPO
											ZA8->ZA8_DTEMIS := SF2->F2_EMISSAO
											ZA8->ZA8_HREMIS := SF2->F2_HORA
											ZA8->ZA8_CHAVE  := SF2->F2_CHVNFE
											ZA8->ZA8_CODTRA := SF2->F2_TRANSP
											ZA8->ZA8_PLACA  := SF2->F2_PLACA
											If Empty(_cFilSA4)
												ZA8->ZA8_NOMTRA := Posicione("SA4",1,xFilial("SA4")+ SF2->F2_TRANSP,"A4_NOME")
											Else
												ZA8->ZA8_NOMTRA := Posicione("SA4",1,ZA8->ZA8_CODFIL+SF2->F2_TRANSP,"A4_NOME")
											Endif
											ZA8->ZA8_USRINC := UsrRetName(RetCodUsr())
											ZA8->(MsUnlock())
										Endif
									Endif
									// Grava quando a nota for Transferencia entre filiais
...
If ntipo <> 1
	...
	// Ricardo 22/07/2018 - Fonte auxiliar
	PRIVATE oFont06    := TFontEx():New(oDanfe,"Times New Roman",05,05,.F.,.T.,.F.)// 3
	...
	// Ricardo 22/07/2018 - Fonte auxiliar
	PRIVATE oFont16N   := TFontEx():New(oDanfe,"Times New Roman",15,15,.T.,.T.,.F.)// 12
...
If Type("oTransp:_Transporta")<>"U"
   ...
   // Ricardo 30/07/2018 - Solict Tiago - Forçar a exibição de dados do veículo ainda que não seja permitido no XML da NFE 4.0
   if empty(aTransp[04]).and.!empty(sf2->f2_placa)
   
      // SF2 já está posicionado
      aTransp[04]:=sf2->f2_placa
      _cAliasAtu:=alias()
      Dbselectarea("SA4")
      /*
      Sa4->(dbSetOrder(1))
      if (sa4->(DbSeek(xFilial()+SF2->F2_TRANSP,.f.)))
         aTransp[05]:=SA4->A4_EST
         //aTransp[]:=SA4->A4_YANTT
      endif   
      if !empty(_cAliasAtu)
...
If Type("oNF:_InfNfe:_infAdic:_infAdFisco:TEXT")<>"U"
	...
	EndDo
	*/
	QuebraMENS( cAux, @aMensagem, MAXMENLIN, "-MENS" )
EndIf
...
If Type("oNF:_InfNfe:_infAdic:_infCpl:TEXT")<>"U"
	...
	*/
	If SF1->F1_EST = "EX"
		QuebraMENS( cAux, @aMensagem, MAXMENLIN, "-MENS" )
	Else
		U_AcDAdic( @aMensagem, cAux, MAXMENLIN)
	Endif
EndIf
/*
dbSelectArea("SF1")
...
*/
oDanfe:Box(000,000,010,501)
oDanfe:Say(006, 002, "RECEBEMOS DE "+NoChar(oEmitente:_xNome:Text,lConverte)+" OS PRODUTOS CONSTANTES DA N.FISCAL INDICADA AO LADO", oFont06:oFont)
oDanfe:Box(009, 000,047,101)
oDanfe:Say(017, 002, "DATA DE RECEBIMENTO", oFont07N:oFont)
oDanfe:Box(009, 080,047,400)  // OK BOX - DATA DE RECEBIMENTO
_cCanhoto := u_DanfeCAnhoto(aItens[1,2],_cCodCli,_cLojaCli)
oDanfe:Say(017, 082, NoChar(Substr(_cCanhoto,01,80),lConverte), oFont07:oFont)  //ALTERADO AQUI O FONTE ORIGINAL
oDanfe:Say(027, 082, NoChar(Substr(_cCanhoto,81,80),lConverte), oFont07:oFont)  //ALTERADO AQUI O FONTE ORIGINAL
oDanfe:Say(037, 082, "IDENTIFICAÇÃO E ASSINATURA DO RECEBEDOR", oFont07N:oFont)
oDanfe:Box(000,400,047,603)
oDanfe:Say(008, 437, "NF-e : "+StrZero(Val(oIdent:_NNf:Text),9)+" Série : "+oIdent:_Serie:Text, oFont10:oFont)
nFontSize := 24
oDanfe:Code128C(038,410,SubStr(oNF:_InfNfe:_ID:Text,4), nFontSize )
...
/* Ricardo 06/10/2019 - Remoção da remoção da formatação - Solict Edson
// Ricardo 06/10/2019 - Remover formatação - Solicit Edson
_nLimMens:=88
_cMensCompleta:=""
aeval(aMensagem,{|_cLinha|_cMensCompleta+=alltrim(_cLinha)+" "})
_vNewMens:={}
for _nVez:=1 to mlcount(_cMensCompleta,_nLimMens)
    aadd(_vNewMens,memoline(_cMensCompleta,_nLimMens,_nVez))
next
aMensagem:=aClone(_vNewMens)  
nLenMensagens:= Len(aMensagem)
// Fim da remoção da formatação
*/
For nX := 1 To Min(nLenMensagens, MAXMSG)
    // Ricardo 06/10/2019 - Omitir linhas em branco
    _cImp:=aMensagem[nX]
    if !empty(_cImp)
		oDanfe:Say(nLin,002,_cImp,oFont08:oFont)
		nLin:= nLin+10
	endif	
Next nX
nMensagem := nX
...
//	aTamCol := RetTamCol(aAuxCabec, aAux, oDanfe, oFont08:oFont, oFont08N:oFont)
              ...
              // Ricardo 21/08/2019
If !_lJob.and.!IsInCallStack("U_TJOBSRVCONTROL")
	// Retorna o tamanho das colunas baseado em seu conteudo
	aTamCol := RetTamCol(aAuxCabec, aAux, oDanfe, oFont08:oFont, oFont08N:oFont)
Else
	// Retorna o tamanho das colunas baseado em seu conteudo, quando executado via Job, pois a função FWFontSize não funciona via JOB
	ATAMCOL := {55,172,40,16,24,14,39,53,38,36,34,27,31,24}
Endif
...
If _lJob
	_aBoleto := {2,"",SF2->F2_SERIE,SF2->F2_SERIE,SF2->F2_DOC,SF2->F2_DOC,0,0,ctod('')}
	
	U_BRI097(_aBoleto,@oDanfe)
Endif
...
do case
	...
	Static Function QuebraMens( cAux, aMensagem, _nTamMen, cFlagQuebra )
	LOCAL _nPos:=0
	DEFAULT cFlagQuebra:="MENS-"
	
	While !Empty(cAux)
		_nPos := AT((cFlagQuebra),cAux)
		If _nPos <= _nTamMen .And. _nPos >= 1
			aadd(aMensagem,SubStr(cAux,1,_nPos-1))
			cAux := Substr(cAux,_nPos+5,Len(cAux))
		Else
			aadd(aMensagem,SubStr(cAux,1,_nTamMen))
			cAux := SubStr(cAux,_nTamMen+1,Len(cAux))
		Endif
	EndDo
