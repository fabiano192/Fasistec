...
User Function XmlNfeSef(cTipo,cSerie,cNota,cClieFor,cLoja,cNotaOri,cSerieOri)
	...
	Local aAgrICMS 		:= {}									// Verifica se a TES utiliza agrega ICMS   para incluir o valor na Tag vOutros
			...
			If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
				...
				Else
					...
					*/
					...
					dbSelectArea("SZ2")
					dbsetOrder(1)
					If dbSeek(xFilial("SZ2")+SF2->F2_YPLACA)
						If SF2->F2_YFRETE == "F" .And. SZ2->Z2_CLI + SZ2->Z2_LOJA == SF2->F2_CLIENTE + SF2->F2_LOJA
							_lCarProp := .T.
						Endif
					Endif
					...
					If !Empty(SF2->F2_TRANSP)
						...
						If !Empty(SF2->F2_YLJTRAN)
							_cLjTrans := SF2->F2_YLJTRAN
						Endif

						If !SF2->F2_TIPO $ "D/B"
							_cNumNf   := StrZero(Val(SF2->F2_DOC),6)

							SZ7->(dbSetOrder(4))
							If SZ7->(dbSeek(SF2->F2_FILIAL + _cNumNf + SF2->F2_SERIE))
								_cTransp  := SZ7->Z7_FORNECE
								_cLjTrans := SZ7->Z7_LOJAF
							Else
								SZ7->(dbSetOrder(9))
								If SZ7->(dbSeek(SF2->F2_FILIAL + _cNumNf + SF2->F2_SERIE))
									_cTransp  := SZ7->Z7_FORNECE
						...
						SA2->(dbSetOrder(1))
						If SA2->(MsSeek(xFilial("SA2") + _cTransp + _cLjTrans))

							aadd(aTransp,SA2->A2_CGC)
							...
							aadd(aTransp,SA2->A2_EMAIL	)

							dbSelectArea("SZ2")
							dbsetOrder(1)
							If dbSeek(xFilial("SZ2")+SF2->F2_YPLACA)

								aadd(aVeiculo,SF2->F2_YPLACA)
								aadd(aVeiculo,SZ2->Z2_UF)
								aadd(aVeiculo,SZ2->Z2_RNTRC)
								//aadd(aVeiculo,"")//RNTC

								If !Empty(SF2->F2_YPLCAR)
									If RIGHT(SF2->F2_YPLCAR,4) <= "9999"
										aadd(aReboque,SF2->F2_YPLCAR)
										aadd(aReboque,SZ2->Z2_UF)
										aadd(aReboque,SZ2->Z2_RNTRC)
										//aadd(aReboque,"") //RNTC
									ElseIf Alltrim(SF2->F2_YPLCAR) $ "CARRETA/BITREM" // Se PLCAR =  CARRETA ou BITREM
										aadd(aReboque,SF2->F2_YPLACA)
										aadd(aReboque,SZ2->Z2_UF)
										aadd(aReboque,SZ2->Z2_RNTRC)
										//aadd(aReboque,"") //RNTC
									Endif
								...
								/*dbSelectArea("SZ8")
								dbSetOrder(1)
								If SZ8->(FieldPos("Z8_PLCAR2")) > 0
								cPLCAR2 := Posicione("SZ8",1,xFilial("SZ8")+Alltrim(SZ7->Z7_OC),"Z8_PLCAR2")
								Endif
								If !empty(cPLCAR2)
								aadd(aReboqu2,cPLCAR2)
								aadd(aReboqu2,SZ2->Z2_UF)
								aadd(aReboqu2,SZ2->Z2_RNTRC)
								//aadd(aReboqu2,"") //RNTC
								Endif*/ //Marcus Vinicius - 12/05/2017 - Desabilitado

								//Marcus Vinicius - 12/05/2017 - Ajustado a forma de coletar os dados dos campos F2_YPLCAR2 pois antes posicionava na SZ8, nesse momento busca direto do campo do cabeçalho da nota fiscal.
								if !empty(SF2->F2_YPLCAR2)
									aadd(aReboqu2,SF2->F2_YPLCAR2)
									aadd(aReboqu2,SZ2->Z2_UF)
									aadd(aReboqu2,SZ2->Z2_RNTRC)
									//aadd(aReboqu2,"") //RNTC
								endif
					...
					If Empty(SF2->F2_TRANSP) .AND. (SF2->F2_TIPO == "N")
						...
						If !_lCarProp
							SZ7->(DbSetOrder(4))
							If SZ7->(DbSeek(xFilial("SZ7")+Substr((cAliasSD2)->D2_DOC,1,6)+(cAliasSD2)->D2_SERIE))

								SZ3->(DbSetOrder(1))
								If SZ3->(DbSeek(xFilial("SZ3")+SZ7->Z7_MOTOR))
									aadd(aTransp,SoNumeros(SZ3->Z3_CPF))
									aadd(aTransp,SZ3->Z3_NOME)
									aadd(aTransp,"")
									aadd(aTransp,SZ3->Z3_END)
									aadd(aTransp,SZ3->Z3_MUN)
									aadd(aTransp,Upper(SZ3->Z3_EST)	)
									aadd(aTransp,"")
									...
									IF dbSeek(xFilial("SZ2")+SF2->F2_YPLACA)
										...
										If !Empty(SF2->F2_YPLCAR)
											If RIGHT(SF2->F2_YPLCAR,4) <= "9999"
												...
												aadd(aReboque,SZ2->Z2_UF)
												aadd(aReboque,SZ2->Z2_RNTRC)
											ElseIf Alltrim(SF2->F2_YPLCAR) $ "CARRETA/BITREM"
												aadd(aReboque,SF2->F2_YPLACA)
												aadd(aReboque,SZ2->Z2_UF)
												aadd(aReboque,SZ2->Z2_RNTRC)
												//aadd(aReboque,"") //RNTC
							...
							Else
								...
								If SZ7->(DbSeek(xFilial("SZ7")+Substr((cAliasSD2)->D2_DOC,1,6)+(cAliasSD2)->D2_SERIE))
									...
									If SZ3->(DbSeek(xFilial("SZ3")+SZ7->Z7_MOTOR))
										...
										aadd(aTransp,SZ3->Z3_MUN)
										aadd(aTransp,Upper(SZ3->Z3_EST)	)
										aadd(aTransp,"") // FALTA CRIAR O CAMPO DE E-MAIL NA TABELA SZ3
							...
							Endif
						Else
							aadd(aTransp,SoNumeros(SA1->A1_CGC))
							aadd(aTransp,SA1->A1_NOME)
							aadd(aTransp,"")
							aadd(aTransp,SA1->A1_END)
							aadd(aTransp,SA1->A1_MUN)
							aadd(aTransp,Upper(SA1->A1_EST)	)
							aadd(aTransp,"")
					...
					EndDo
					*/
					//VOLUMES PERSONALIZADO PARA MIZU

					cEspecie := Upper(FieldGet(FieldPos("F2_ESPECI1")))
					//			If !Empty(SF2->F2_TRANSP)		// Desabilitado por Marcus Vinicius 08/07/2015 - Deve-se sempre sair o peso liquido e bruto sendo zerado ou não
					//               If !Empty(cEspecie)
					aadd(aEspVol,{ cEspecie, FieldGet(FieldPos("F2_VOLUME1")) , SF2->F2_PLIQUI , SF2->F2_PBRUTO})
					//			     Endif
					//			Endif							// Desabilitado por Marcus Vinicius 08/07/2015 - Deve-se sempre sair o peso liquido e bruto sendo zerado ou não
					//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
					//³Procura duplicatas                                                      ³
					//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
					...
					If !Empty(SF2->F2_DUPL)
						...
						SF2->F2_DOC == (cAliasSE1)->E1_NUM
							If (cAliasSE1)->E1_TIPO = MVNOTAFIS .OR. (cAliasSE1)->E1_TIPO = 'DP' .OR. ((Alltrim((cAliasSE1)->E1_ORIGEM) $ 'LOJA701|FATA701|LOJA010') .AND. (cAliasSE1)->E1_TIPO $ cWhere)
								...
								EndIf
								acNosso:=acNosso+Alltrim((cAliasSE1)->E1_NUMBCO)+"/"
							EndIf
					...
					SF2->F2_DOC == (cAliasSD2)->D2_DOC
						...
						If lContinua
							...
							EndIf
							*/
							//POSCIONA ARQUIVO SZ7 - PERSONALIZACAO MIZU
							SZ7->(DbSetOrder(4))
							If SZ7->(!DbSeek(xFilial("SZ7")+Substr((cAliasSD2)->D2_DOC,1,6)+(cAliasSD2)->D2_SERIE))
								SZ7->(DbSetOrder(9))
								SZ7->(!DbSeek(xFilial("SZ7")+Substr((cAliasSD2)->D2_DOC,1,6)+(cAliasSD2)->D2_SERIE))
							Endif
							...
							If Alltrim((cAliasSD2)->D2_ROTINA) == "MIZ999"
								//If cEmpAnt+cFilant == "0203"
								//	If SF4->F4_TIPO == "S" .AND. SF4->F4_DUPLIC == "S" .AND. SF4->F4_TEXTO $ "VENDA|REVENDA"		//INCLUSA FORMULA ATENDENDO AO CHAMADO 39103
								//		cMensFis += FORMULA("242")
								//	Endif
								If cEmpAnt + cFilAnt == "0210" .And.  (cAliasSD2)->D2_VALICM = 0
									If SA1->A1_TIPO == "F"     .And. SF4->F4_TPMOV = "F" .And. SA1->A1_CONTRIB == "2" .And. SA1->A1_EST = "MA"
										cMensFis += Alltrim(FORMULA("244"))
									ElseIf SA1->A1_TIPO == "F" .And. SF4->F4_TPMOV = "F" .And. SA1->A1_CONTRIB == "2" .And. SA1->A1_EST <> "MA"
										cMensFis += Alltrim(FORMULA("245")) // LIMINAR PARTILHA ICMS 80 % - BARAUNA
									Endif
								ElseIf cEmpAnt + cFilAnt == "0213" .And.  (cAliasSD2)->D2_VALICM = 0
									If SA1->A1_TIPO == "F" .And. SF4->F4_TPMOV = "F" .And. SA1->A1_CONTRIB == "2"
										cMensFis += Alltrim(FORMULA("247"))+Alltrim(FORMULA("248")) // LIMINAR PARTILHA ICMS 80 % - PACATUBA - SERGIPE
									Endif
								Endif
							Else
								If !AllTrim(SC5->C5_MENNOTA) $ cMensCli
								...
								If (cAliasSD2)->D2_TIPO == "B"
									_cMensBen += "/"+(cAliasSD2)->D2_NFORI+"-"+(cAliasSD2)->D2_ITEMORI
								Endif
							Endif

							If cEmpAnt+cFilant == "0203"
								cMensFis += Alltrim(FORMULA("999"))
								If SF4->F4_TIPO == "S" .AND. SF4->F4_DUPLIC == "S" .AND. Alltrim((cAliasSD2)->D2_TIPO) == "N"		//INCLUSA FORMULA ATENDENDO AO CHAMADO 39103
									cMensFis += Alltrim(FORMULA("242"))
								Endif
							Endif

							If cEmpAnt $ "02"
								If SF4->F4_DUPLIC == "S" .AND. SF4->F4_TIPO == "S" .AND. SF4->F4_PISCOF == "3" .AND. SF4->F4_PISCRED == "2" .AND. Alltrim((cAliasSD2)->D2_TIPO) == "N"  //INCLUSA FORMULA ATENDENDO CHAMADO 42837
									cMensFis += Alltrim(FORMULA("508"))
								Endif
							Endif
							...
							If !Empty(SA1->A1_SUFRAMA)
								cMensFis += " Codigo SUFRAMA: "+SA1->A1_SUFRAMA
							Endif

							IF (SF2->F2_TIPO == "N")
								cMensCli01 := SZ7->Z7_MENS01
								IF !Empty(cMensCli01)
									SM4->(DbSetOrder(1))
									If SM4->(DbSeek(xFilial("SM4")+cMensCli01,.F.))
										If SM4->M4_YUSADO == "S"
											IF !Empty(cMensCli)
												cMensCli+="#"
											ENDIF
											cMensCli += Formula(cMensCli01)
										Endif
									Endif
								ENDIF

								cMensCli01 := SZ7->Z7_MENS02
								IF !Empty(cMensCli01)
									SM4->(DbSetOrder(1))
									If SM4->(DbSeek(xFilial("SM4")+cMensCli01,.F.))
										If SM4->M4_YUSADO == "S"
							...
							If _lCarProp
								...
								cMensCli += "TRANSPORTE DE CARGA PROPRIA#"
								cMensCli +=	"MOTORISTA: "+Alltrim(SZ3->Z3_NOME)
							Endif
							...
							if !empty(cPLCAR2)
							cPLACAS += " / " + Alltrim(cPLCAR2)
							endif*/ //Marcus Vinicius - 12/05/2017 - Desabiiitado

							//Marcus Vinicius - 12/05/2017 - Ajustado a forma de coletar os dados dos campos F2_YPLCAR2 pois antes posicionava na SZ8, nesse momento busca direto do campo do cabeçalho da nota fiscal.
							IF !Empty(SF2->F2_YPLCAR) .OR. !Empty(SF2->F2_YPLCAR2) //.OR. !empty(cPLACAS)

								if  SZ7->Z7_COMDESC == "S"

									cMensCli01 := "Carreta Placa(D): "+Alltrim(SF2->F2_YPLCAR) + " / " + Alltrim(SF2->F2_YPLCAR2)
								else

									cMensCli01 := "Carreta Placa: "+Alltrim(SF2->F2_YPLCAR) + " / " + Alltrim(SF2->F2_YPLCAR2)
							...
							IF SZ7->Z7_TRGR == "S"
								cMensCli01 := "Gerada Nota de Frete a Granel"
								IF !Empty(cMensCli01)
									IF !Empty(cMensCli)
										cMensCli+="#"
									ENDIF
							...
							IF !Empty(acNosso)
								cMensCli01 := "Nosso Numero: " + acNosso
								IF !Empty(cMensCli01)
									IF !Empty(cMensCli)
										cMensCli+="#"
									ENDIF
							...
							IF !Empty(SZ7->Z7_LACRE)
								cMensCli01 := "Numero do lacre: " +Alltrim(SZ7->Z7_LACRE)
								IF !Empty(cMensCli01)
									IF !Empty(cMensCli)
										cMensCli+="#"
									ENDIF
							...
							If !Empty(SZ7->Z7_OBSER)
								cMensCli+="#"
								cMensCli+= Alltrim(SZ7->Z7_OBSER)
							Endif

							If SF4->F4_TPMOV = "Y" //Pedido Triangular
								_aAliasSA1 := SA1->(GetArea())
								SA1->(dbSetOrder(1))
								If SA1->(msSeek(xFilial("SA1")+SZ7->Z7_OBSA))
									cMensCli+="#"

									cMensCli+= "Nota fiscal emitida de acordo como Artigo 459 paragrafo 1º. Mercadoria entregue no estabelecimento de nome: "+Alltrim(SA1->A1_NOME)
									cMensCli+="#"
									cMensCli+= "CNPJ: "+Transform(SA1->A1_CGC,"@R 99.999.999/9999-99")+" - IE: "+SA1->A1_INSCR
									cMensCli+="#"
									cMensCli+= "Endereço: "+Alltrim(SA1->A1_END)+" - "+Alltrim(SA1->A1_MUN)+" - "+Alltrim(SA1->A1_EST)
									cMensCli+="#"
									cMensCli+= "Bairro: "+Alltrim(SA1->A1_BAIRRO)+" - CEP: "+Transform(SA1->A1_CEP,"@R 99999-999")
								Endif
								RestArea(_aAliasSA1)
							ElseIf SF4->F4_TPMOV = "Z"   //Pedido Triangular - Remessa - Marcus Vinicius - 22/02/2017
								_aAliasSZ7 := SZ7->(GetArea())
								SZ7->(dbOrderNickName("INDOPTRI"))
								If SZ7->(msSeek(xFilial("SZ7")+SD2->D2_PEDIDO+SD2->D2_CLIENTE+SD2->D2_LOJA))
									_aAliasSA1 := SA1->(GetArea())
									SA1->(dbSetOrder(1))
									IF SA1->(msSeek(xFilial("SA1")+SZ7->Z7_CLIENTE+SZ7->Z7_LOJA))
										cMensCli+="#"
										cMensCli+= "Nota fiscal emitida de acordo como Artigo 459 paragrafo 1º. Mercadoria faturada para o estabelecimento de nome: "+Alltrim(SA1->A1_NOME)
										cMensCli+="#"
										cMensCli+= "CNPJ: "+Transform(SA1->A1_CGC,"@R 99.999.999/9999-99")+" - IE: "+SA1->A1_INSCR
										cMensCli+="#"
										cMensCli+= "Endereço: "+Alltrim(SA1->A1_END)+" - "+Alltrim(SA1->A1_MUN)+" - "+Alltrim(SA1->A1_EST)
							...
							IF !Empty(SZ7->Z7_LOCAL) .and. (SF2->F2_TIPO == "N")
								cMensCli01 := "End. entrega: "+Alltrim(SZ7->Z7_LOCAL)
								IF !Empty(cMensCli01)
									IF !Empty(cMensCli)
										cMensCli+="#"
									ENDIF
							...
							ZA2->(DBSelectArea("ZA2"))
							ZA2->(dbSetOrder(2))
							ZA2->(dbSeek(xFilial("ZA2") + SZ7->Z7_CLIENTE + SZ7->Z7_LOJA + SZ7->Z7_PRODUTO + SF2->F2_YOBRA))
							If ZA2->(Found())
								If !Empty(ZA2->ZA2_OBSNF)
									cMensCli +="#"

									If  "FORMULA(" $ ZA2->ZA2_OBSNF
										_cForm := ZA2->ZA2_OBSNF
										cMensCli +="Obs. Obra: "+&(_cForm)
									Else
										cMensCli +="Obs. Obra: "+Alltrim(ZA2->ZA2_OBSNF)
									EndIf
								Endif
							Endif

							If Posicione("SZ3",1,xFilial("SZ3")+SZ7->Z7_MOTOR,"FOUND()")
								If SF2->F2_FRETE > 0  .and. SZ3->Z3_TIPO == "2" //.and. SZ3->Z3_AGREGA <> "S"
									If SM4->(DbSeek(xFilial("SM4")+"010",.F.))
										If SM4->M4_YUSADO == "S"

											cMensCli01 := Formula("010")
											IF !Empty(cMensCli01)
												IF !Empty(cMensCli)
													cMensCli+="#"
												ENDIF
											...
											cMensCli01 := "VALOR DO FRETE : "+Alltrim(AllTrim(Transform(SF2->F2_FRETE,"@e 999,999.99")))+"   ALIQUOTA : 12%   ICMS A RECOLHER : "+Alltrim(Transform(Round((SF2->F2_FRETE*12)/100,3),"@e 999,999.99"))
											IF !Empty(cMensCli01)
												IF !Empty(cMensCli)
													cMensCli+="#"
												ENDIF
							...
							//Mensagem referente a tranportadora sem CTR no estado de ORIGEM da venda
							IF  cEmpAnt + cFilAnt $ '0210|3001' .AND. SZ7->Z7_FORNECE+SZ7->Z7_LOJAF $ AllTrim(GetNewPar("MZ_TRANSIE",""));
							.AND. SF2->F2_EST <> "RN"
								If SM4->(DbSeek(xFilial("SM4")+"010",.F.))
									If SM4->M4_YUSADO == "S"
							...
							_lSA7 := .F.
							SA7->(dbSetOrder(1))
							If SA7->(dbSeek(xFilial("SA7") + SF2->F2_CLIENTE + SF2->F2_LOJA + (cAliasSD2)->D2_COD ))
								If SA7->A7_YFRETE == "1"
									cModFrete := "0"
									_lSA7     := .T.
								Endif
							...
							If  !_lSA7
								If _lachou
									cModFrete := IIF(SC5->C5_TPFRETE=="C","0","1")
								Else
									cModFrete := IIF(SF2->F2_YFRETE=="C","0","1")
								Endif
							Endif
							...
							If (SC6->(FieldPos("C6_NUMPCOM")) > 0 .And. SC6->(FieldPos("C6_ITEMPC")) > 0) .OR. (SZ7->(FieldPos("Z7_YPEDCOM")) > 0 .And. SZ7->(FieldPos("Z7_YITEMPC")) > 0)
								If !Empty(SC6->C6_NUMPCOM) .And. !Empty(SC6->C6_ITEMPC)
									aadd(aPedCom,{SC6->C6_NUMPCOM,SC6->C6_ITEMPC})
								ElseIf !Empty(SZ7->Z7_YPEDCOM) .And. !Empty(SZ7->Z7_YITEMPC) 	// Marcus Vinicius 22/12/2015
									aadd(aPedCom,{SZ7->Z7_YPEDCOM,SZ7->Z7_YITEMPC})			// Preencher a tag xPed quando utilizado pedido de venda mizu.
								Else														//
									aadd(aPedCom,{})										// Até aqui
								EndIf
							Else
								aadd(aPedCom,{})
							EndIf
							...
							If SF2->F2_TIPO $ "B/D" .Or. SF2->F2_EST = "EX"
								_cDescri := SC6->C6_DESCRI
								//					ElseIf !Alltrim(FunName()) $ "MIZ999/MIZ022"
							ElseIf !TRIM((cAliasSD2)->D2_ROTINA) $ "MIZ999/MIZ022"	//Marcus Vinicius 17/08/16
								If 	SF2->F2_TIPO == "N" .AND. SC5->C5_TIPO == "N"   //    INCLUSO POR ALISON	24/02/15
									_cDescri := SC6->C6_DESCRI
								Else
						...
						IIF(Empty(SB5->B5_CONVDIP),(cAliasSD2)->D2_QUANT,SB5->B5_CONVDIP*(cAliasSD2)->D2_QUANT),;
							...
							Alltrim((cAliasSD2)->D2_COD) == Alltrim(CD2->CD2_CODPRO)
								...
								Do Case
									...
									Iif(cVerAmb == "4.00" .and. FindFunction("xFisRetFCP"), xFisRetFCP('4.0','CD2','CD2_ALIQ'), If(lNfCupZero,0,Iif(CD2->CD2_BC>0,CD2->CD2_ALIQ,0))),;
									...
									If aAgrICMS[Len(aAgrICMS)][1]
										aAgrICMS[Len(aAgrICMS)][2] := CD2->CD2_VLTRIB
									EndIf
									...
									If lConsig .And. (Alltrim((cAliasSD2)->D2_CF) $ cMVCFOPREM)  .And. CD2->CD2_VLTRIB > 0
										aTail(aICMSST):= {CD2->CD2_ORIGEM,CD2->CD2_CST,CD2->CD2_MODBC,0,0,0,0,CD2->CD2_MVA,0,CD2->CD2_PAUTA,Iif(lCD2PARTIC,CD2->CD2_PARTIC,""),IIf(CD2->(ColumnPos("CD2_DESONE"))>0,CD2->CD2_DESONE,0),IIf(CD2->(ColumnPos("CD2_BFCP")) > 0,CD2->CD2_BFCP,0),IIf(CD2->(ColumnPos("CD2_PFCP")) > 0,CD2->CD2_PFCP,0),IIf(CD2->(ColumnPos("CD2_VFCP")) > 0,CD2->CD2_VFCP,0)}
									EndIf
									lCalSol := .T.
									//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
									...
									If !lConsig
										...
										SB1->B1_CLASSE,;
										0,;
									"999",;//IIf(CD2->(FieldPos("CD2_GRPCST")) > 0 .and. !Empty(CD2->CD2_GRPCST),CD2->CD2_GRPCST,"999"),; //NT2015/002
										CD2->CD2_CST,;
										CD2->CD2_BC,;
										CD2->CD2_QTRIB,;
										CD2->CD2_PAUTA,;
							...
							If cEmpAnt + cFilAnt $ "0210/0213" .And.  (cAliasSD2)->D2_VALICM = 0 .And. SA1->A1_EST <> SM0->M0_ESTCOB   // LIMINAR BARAUNA / SERGIPE
								If SA1->A1_TIPO == "F" .And. SF4->F4_TPMOV = "F" .And. SA1->A1_CONTRIB == "2" .And. SA1->A1_EST <> "MA"

									aTail(aICMUFDest) := 	{(cAliasSD2)->D2_BASEDES,;  //[1]vBCUFDest      CD2_BC
									0,;							//[2]pFCPUFDest     CD2_PFCP
									(cAliasSD2)->D2_ALIQCMP ,;  //[3]pICMSUFDest    CD2_ALIQ
									12,;                      	//[4]pICMSInter     CD2_ADIF
									(cAliasSD2)->D2_PDDES,;    	//[5]pICMSInterPart CD2_PDDES 80%
									0,;   						//[6]vFCPUFDest 	CD2_VFCP
									(cAliasSD2)->D2_DIFAL,;		//[7]vICMSUFDest    CD2_VDDES Valor 80%
									0}                          //[8]vICMSUFRemet   CD2_VLTRIB
									lCalSol := .T.
								Endif
							Endif


							//Gerar a mensagem do ICMS Partilhado
							If (cAliasSD2)->D2_ICMSRET > 0 .And. SA1->A1_CONTRIB = '2' .AND. SA1->A1_EST <> SM0->M0_ESTCOB
								_nICMOrig += (cAliasSD2)->D2_ICMSRET * 0.20
								_nICMDest += (cAliasSD2)->D2_ICMSRET * 0.80
							Endif
	...
	Else
		...
		If MsSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
			...
			Else
				//------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
				If Empty(SF1->F1_HORA) // Marcus Vinicius 13/07/2017 - Ajustado para atender Manaus quando devolução de nota fiscal devido o fuso horario
					_dEmissao:= dDataBase
					_cHora   := Left(time(),5)
					...
					aadd(aNota,dDataBase)
					aadd(aNota,cTipo)
					aadd(aNota,SF1->F1_TIPO)
					aadd(aNota,_cHora,5)
					// Marcus Vinicius - 13/07/2017 - Até aqui
					//------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
				Else
					aadd(aNota,SF1->F1_SERIE)
					aadd(aNota,IIF(Len(SF1->F1_DOC)==6,"000","")+SF1->F1_DOC)
				...
				If !Empty(XPLACA)
					AADD(_aPlaca,XPLACA)
					aadd(aVeiculo,XPLACA)
					aadd(aVeiculo,XPLACAUF)
					aadd(aVeiculo,"")//RNTC
				Endif

				If SF1->(FieldPos("F1_MENSNF")) > 0
					If !Empty(SF1->F1_MENSNF)
						cMensCli += AllTrim(SF1->F1_MENSNF)+"#"
					EndIF
				Endif
				...
				If SF1->(FieldPos("F1_MENPAD"))>0
					If !Empty(SF1->F1_MENPAD)
						cMensFis += AllTrim(FORMULA(SF1->F1_MENPAD))+="#"
					EndIf
				EndIf

				If SF1->(FieldPos("F1_YMENSNF"))>0
					If !Empty(SF1->F1_YMENSNF)
						cMensFis += AllTrim(SF1->F1_YMENSNF)+="#"
					EndIf
				EndIf
				...
				If SF1->F1_EST = "EX"
					...
					If PA2->(dbSeek(xFilial("PA2")+ SF1->F1_FORNECE + SF1->F1_LOJA + SF1->F1_DOC + SF1->F1_SERIE ))
						...
						While PA2->(!Eof()) .And.	_cChavPA2 == PA2->PA2_FORREM + PA2->PA2_LJREM + PA2->PA2_NFREM + PA2->PA2_SERREM
							...
							_cMensPA2 += " * "+Alltrim(PA2->PA2_PACAGE)
					...
					Else
						...
						If PA1->(dbSeek(xFilial("PA1")+ SF1->F1_FORNECE + SF1->F1_LOJA + SF1->F1_DOC + SF1->F1_SERIE ))
							...
							While PA1->(!Eof()) .And. _cChavPA1 == PA1->PA1_FORREM + PA1->PA1_LJREM + PA1->PA1_NFREM + PA1->PA1_SERREM
								...
								_cMensPA1 += "/"+Alltrim(PA1->PA1_PACAGE)
				...
				SF1->F1_LOJA ==  (cAliasSD1)->D1_LOJA
					...
					Else
						aAdd(aAgrCofins,{.F.,0})
					EndIf
				aAdd(aAgrICMS,{.F.,0})          // Marcus Vinicius - 08/06/2016
					...
					If ( nScan2 == 0 )
						...
						If !Empty(SF4->F4_FORMULA)
							cMensCli += Alltrim(Formula(SF4->F4_FORMULA) )+"MENS-"
						EndIf

						If !Empty(SF4->F4_FORMU04)
							cMensCli += Alltrim(Formula(SF4->F4_FORMU04))+"MENS-"
						EndIf
					Endif

					nScan2  := aScan(_aGrpMen3,SF4->F4_CODIGO)
					If ( nScan2 == 0 )
						AADD(_aGrpMen3,SF4->F4_CODIGO)

						If (cAliasSD1)->D1_VALIMP1 > 0    /// I.I.
							If  !Empty(SF4->F4_FORMU05)
								cMensFis += Alltrim(Formula(SF4->F4_FORMU05))+"MENS-"
							EndIf
						Endif

						If (cAliasSD1)->D1_VALIMP6 > 0    /// PIS
							If  !Empty(SF4->F4_FORMU01)
								cMensFis += Alltrim(Formula(SF4->F4_FORMU01))+"MENS-"
							EndIf
						Endif

						If (cAliasSD1)->D1_VALIMP5 > 0    /// COFINS
							If !Empty(SF4->F4_FORMU02)
								cMensFis += Alltrim(Formula(SF4->F4_FORMU02))+"MENS-"
							EndIf
						Endif

						If !Empty(SF4->F4_FORMU03)
							cMensFis += Alltrim(Formula(SF4->F4_FORMU03))+"MENS-"
						EndIf
					Endif
					...
					If SF1->F1_TIPO == "C"
						_nQtde := 1
					Else
						_nQtde := (cAliasSD1)->D1_QUANT
					Endif
					_cDescri := (cAliasSD1)->D1_DESCRI
	...
	If !Empty(aNota)
		...
		Next nX
	cString += NfeTotal(aTotal,aRetido,aICMS,aICMSST,lIcmDevol,cVerAmb,aISSQN,nVicmsDeson,aNota,nVIcmDif,aAgrPis,aAgrCofins,aAgrICMS)
...
Static Function NfeItem(aProd,aICMS,aICMSST,aIPI,aPIS,aPISST,aCOFINS,aCOFINSST,aISSQN,aCST,aMed,aArma,aveicProd,aDI,aAdi,aExp,aPisAlqZ,aCofAlqZ,aAnfI,cTipo,cVerAmb,aComb,cMensFis,cCsosn,aPedCom,aNota,aICMSZFM,aDest,cIpiCst,aFCI,lIcmDevol,nVicmsDeson,nVIcmDif,cMunPres,aAgrPis,aAgrCofins,nIcmsDif,aICMUFDest,nvFCPUFDest,nvICMSUFDest,nvICMSUFRemet,cAmbiente,aIPIDevol,nvBCUFDest, aItemVinc,npFCPUFDest,npICMSUFDest,npICMSInter,npICMSIntP,aLote)
	...
	If Len(aDI)>0 .And. ConvType(aDI[04][1]) == "I19"
		...
		*/
	ElseIf _cEst == "EX"
		_cData := Substr(dtos(DDATABASE),1,4)+"-"+Substr(dtos(DDATABASE),5,2)+"-"+Substr(dtos(DDATABASE),7,2)
		dbSelectArea("SA2")
		dbSetOrder(1)
		MsSeek(xFilial("SA2")+SF1->F1_FORNECE+SF1->F1_LOJA)

		cString += '<DI>'
		cString += '<nDI>'+"9999999999"+'</nDI>'
		cString += '<dtDi>'+_cData+ '</dtDi>'
		cString += '<LocDesemb>'+"ZZZZZ"+ '</LocDesemb>'
		cString += '<UFDesemb>'+"SP"+ '</UFDesemb>'
		cString += '<dtDesemb>'+_cData+ '</dtDesemb>'

		cString += '<viaTransp>'+'1'+ '</viaTransp>'
		cString += NfeTag('<AFRMM>'+ConvType(0,15,2))
		cString += '<intermedio>'+'1'+ '</intermedio>'
		cString += NfeTag('<CNPJ>'+SM0->M0_CGC)
		cString += NfeTag('<UfTerceiro>'+SM0->M0_ESTCOB)

		cString += '<Exportador>'+Alltrim(SA2->A2_NOME)+ '</Exportador>'

		cString += '<adicao>'
		cString += '<Adicao>'+"1"+ '</Adicao>'
		cString += '<SeqAdic>'+"1"+ '</SeqAdic>'
		cString += '<Fabricante>'+"ZZZ"+ '</Fabricante>'
		cString += '</adicao>'
	...
	*/
	If _cEst == "EX"
		...
		cString += '</Tributo>'
		cString += '<cpl>'
		cString += '<vDespAdu>' +ConvType(0,15,2)+'</vDespAdu>'
		cString += '<vIOF>'     +ConvType(0,15,2)+'</vIOF>'
		cString += '</cpl>'
		cString += '</imposto>'
	Endif
...
Static Function NfeTotal(aTotal,aRet,aICMS,aICMSST,lIcmDevol,cVerAmb,aISSQN,nVicmsDeson,aNota,nVIcmDif,aAgrPis,aAgrCofins,aAgrICMS)
...
Static Function NfeInfAd(cMsgCli,cMsgFis,aPedido,aExp,cAnfavea,aMotivoCont,aNfSa,aNfVinc,aProd,aDI,aNfVincRur,aRet,cNfRefcup,cSerRefcup,cTipo,nIPIConsig,nSTConsig,lBrinde,cVerAmb,aRefECF,nVicmsDeson,nvFCPUFDest,nvICMSUFDest,nvICMSUFRemet,nvBCUFDest,aICMUFDest,nValIpiBene,npFCPUFDest,npICMSUFDest,npICMSInter,npICMSIntP,aObsCont)
	...
	If Len( aNfVinc ) > 0		//Nota de espécie NFE ou NCE ou CTE vinculada
		For nZ := 1  to Len( aNfVinc )
			If !( aNfVinc[nZ][2] + aNfVinc[nZ][3] ) $ cChvNFe
				...
				cNfVinc := ( Alltrim(aNfVinc[nZ][3]) + "/" ) //Retirado a data
	...
	*/
	...
	If _cEst == "EX" .And. _cTipoNf == "S"
		cString += '<exporta>'
		cString += '<UFEmbarq>'+Alltrim(SM0->M0_ESTCOB)+ '</UFEmbarq>'
		cString += '<locembarq>'+Alltrim(SM0->M0_CIDCOB)+ '</locembarq>'
		cString += '</exporta>'
	EndIf
