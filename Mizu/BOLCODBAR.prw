...
±±³Programa  ³  BOLCODBAR ³ Autor ³ Sergio Andre		³ Data ³ 01/02/05 ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descri‡…o ³ BOLETO BRADESCO											   ±±
...
/*/
User Function BolCodBar(aDadosFiltro)
	...
	Private cAgencia	:= GetNewPar("MZ_YAGENCI","23744")

	Private aConta		:=  {{"01"  ,"0045001"},{"02"  ,"0029070"}} 
...
//		IF lBcoNaoOk
		If  _cSa1Bco <> "237"
			ApMsgInfo("O código de banco informado no cadastro do cliente não é 237. Esta rotina imprime apenas boletos para o bradesco. Altere para 237 o banco do cliente e você conseguirá imprimir por aqui.",TRBXXX->(E1_CLIENTE+E1_LOJA+"-"+E1_PREFIXO+E1_NUM+E1_PARCELA) )
			TRBXXX->(dbSkip())
			LOOP
		ENDIF
		
		If SE1->E1_PORTADO <> "237"
			ApMsgInfo("O código de banco informado no título não é 237. Esta rotina imprime apenas boletos para o BRADESCO. Altere para 237 o banco do Título e você conseguirá imprimir por aqui.",TRBXXX->(E1_CLIENTE+E1_LOJA+"-"+E1_PREFIXO+E1_NUM+E1_PARCELA) )
			TRBXXX->(dbSkip())
			LOOP
		Endif
		...
		If UPPER(Alltrim(cUserName)) $ "ALE" .And. DATE() = CTOD("15/10/18")
		...
		Substr(AllTrim(SA6->A6_AGENCIA),1,Len(AllTrim(SA6->A6_AGENCIA))-1),; // [3]Agência
...
Static Function fTransfTrb(wAliasQRY)
	...
	While !(wAliasQRY)->(Eof())
		...
		For i:= 1 to (wAliasQRY)->(FCount())
			If !TRBXXX->(Alltrim(FieldName(i))) $ "R_E_C_N_O_" 
