...
USER Function MZ0118( cAlias, nReg, nOpcx )
	...
	//³Para versoes localizadas em TReport R4 e usado o MATR110A, a chamada e       ³
...
Local nTamCdProd:= TamSX3("C7_PRODUTO")[1]
	...
	//³ExpC5 : Descricao                                                       ³
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	oReport:= TReport():New("MZ0118",cTitle,If(lAuto,Nil,"MTR110"), {|oReport| ReportPrint(oReport,nReg,nOpcx)},STR0001+" "+STR0002)
...
TRCell():New(oSection2,"C7_NUM"		,"SC7","Numero"   ,/*Picture*/,15,/*lPixel*/,,,,,,,.F.)
...
TRCell():New(oSection2,"C7_QUANT"   ,"SC7",/*Titulo*/,PesqPict("SC7","C7_QUANT",12),/*Tamanho*/,/*lPixel*/,/*{|| code-block de impressao }*/,,,,,,.F.)
TRCell():New(oSection2,"C7_SEGUM"   ,"SC7",STR0118,/*Picture*/,/*Tamanho*/,/*lPixel*/,/*{|| code-block de impressao }*/,,,,,,.F.)
TRCell():New(oSection2,"C7_QTSEGUM" ,"SC7",/*Titulo*/,PesqPict("SC7","C7_QUANT",12),/*Tamanho*/,/*lPixel*/,/*{|| code-block de impressao }*/,,,,,,.F.)
...
//	Local nPageWidth  := IIF(nTpImp==1.Or.nTpImp==6,2435,2435) // oReport:PageWidth()
	...
	Local nPrinted    := 0
	Local _nTotLin    := 95 //Variável para definiar o total de linhas
	Local nValIVA     := 0
	Local nTotIpi	  := 0
	Local nTotIcms	  := 0
	Local nTotDesp	  := 0
	...
	Local nTamanCorr:= 149 // tamanho correto do final da linha
...
//	EndIf
	...
	If lAuto
		...
		mv_par05 := Eval({|| cCont:=U_ChkPergUs(cUserId,"MTR110","05"),If(cCont == Nil,2,cCont) })
		mv_par08 := Eval({|| cCont:=U_ChkPergUs(cUserId,"MTR110","08"),If(cCont == Nil,C7_TIPO,cCont) })
		mv_par09 := Eval({|| cCont:=U_ChkPergUs(cUserId,"MTR110","09"),If(cCont == Nil,1,cCont) })
		mv_par10 := Eval({|| cCont:=U_ChkPergUs(cUserId,"MTR110","10"),If(cCont == Nil,3,cCont) })
		mv_par11 := Eval({|| cCont:=U_ChkPergUs(cUserId,"MTR110","11"),If(cCont == Nil,3,cCont) })
		mv_par14 := Eval({|| cCont:=U_ChkPergUs(cUserId,"MTR110","14"),If(cCont == Nil,1,cCont) })
	Else
...
//					oSection2:Cell("C7_DATPRF"):SetSize(9)
					...
					EndIf
				EndIf
			If(oReport:nDevice == 2)
				nTamanCorr := IIF(MV_PAR07 <> 1,nTamanCorr-3,nTamanCorr-2)  // se for impressão por spool diminuir o tamanho da linha
			EndIf
			If(oReport:nDevice == 5)
				nTamanCorr := IIF(MV_PAR07 <> 1,nTamanCorr-2,nTamanCorr-1)  // se for impressão por html diminuir o tamanho da linha
			EndIf
			//Ajusta o tamanho dos campos de acordo com o tamanho do relatorio
		 	If !lArrumou .And. !oSection2:UseFilter()
				...
				For nX:= 1 To Len(aTamCamp) //atualiza o tamanho certo dos campos
			 		If oSection2:Cell(aTamCamp[nX][1]):Enabled()
	   					oSection2:Cell(aTamCamp[nX][1]):SetSize(aTamCamp[nX][2])

   						If nX == 1 .And. oReport:nDevice <> 2
   							If oReport:nDevice == 3
   								oSection2:Cell(aTamCamp[nX][1]):SetSize(aTamCamp[nX][2] - IIF(MV_PAR07 == 1, 2.1, 1))
   							ElseIf oReport:nDevice == 5
								oSection2:Cell(aTamCamp[nX][1]):SetSize(aTamCamp[nX][2] - IIF(MV_PAR07 == 1, 0.6, 0.9))
							ElseIf oReport:nDevice == 6
								oSection2:Cell(aTamCamp[nX][1]):SetSize(aTamCamp[nX][2] - IIF(MV_PAR07 == 1, 0.3, 0.5))
							Else
   								oSection2:Cell(aTamCamp[nX][1]):SetSize(aTamCamp[nX][2] - IIF(MV_PAR07 == 1, 0.2, 0.3))
							EndIf
   						EndIf
			...
			If oReport:Row() > oReport:LineHeight() * 68
				...
				oReport:Box( 280,010,oReport:Row() + oReport:LineHeight() * ( _nTotLin - nPrinted ) , nPageWidth )

			Else
				oReport:Box( oReport:Row(),oReport:Col(),oReport:Row() + oReport:LineHeight() * ( _nTotLin - nPrinted ) , nPageWidth )
			EndIf

			oReport:Box( 1990 ,010,oReport:Row() + oReport:LineHeight() * ( _nTotLin - nPrinted ) , nPageWidth )
			oReport:Box( 2080 ,010,oReport:Row() + oReport:LineHeight() * ( _nTotLin - nPrinted ) , nPageWidth )
			oReport:Box( 2200 ,010,oReport:Row() + oReport:LineHeight() * ( _nTotLin - nPrinted ) , nPageWidth )
			oReport:Box( 2320 ,010,oReport:Row() + oReport:LineHeight() * ( _nTotLin - nPrinted ) , nPageWidth )
...
//				oReport:PrintText(PadR(cMensagem,129), , oSection2:Cell("DESCPROD"):ColPos() )
			...
			If !Empty(SC7->C7_APROV) .Or. (Empty(SC7->C7_APROV) .And. SCR->CR_TIPO == "IP")
				...
				While !Eof() .And. SCR->CR_FILIAL+Alltrim(SCR->CR_NUM) == xFilial("SCR")+Alltrim(SC7->C7_NUM) .And. SCR->CR_TIPO $ "PC|AE|IP"
					...
					If SCR->CR_STATUS <> "03"	// Marcus Vinicius - 14/08/2018 - Incluído para exibir somente quem aprovou o pedido.
						DbSkip()
						Loop
					EndIf
			...
			Else
				...
				// Marcus Vinicius - 14/08/2018 - Ajustado para utilizar os 3 campos de conteúdo do parâmetro MZ_MENSC7
				DbSelectArea("SX6")
				DbSetOrder(1)
				if SX6->(DbSeek(cFilAnt+'MZ_MENSC7'))
					If !Empty(SX6->X6_CONTEUD)
						oReport:PrintText(PADR(SX6->X6_CONTEUD,100),,050 )
					EndIf

					If !Empty(SX6->X6_CONTSPA)
						oReport:PrintText(PADR(SX6->X6_CONTSPA,100),,050 )
					EndIf
					If !Empty(SX6->X6_CONTENG)
						oReport:PrintText(PADR(SX6->X6_CONTENG,100),,050 )
					EndIf
...
Public nRet:= 0
	...
	If(SC7->C7_QTDREEM >= 99)
		...
		If(nRet == 1)
			oReport:PrintText( Str(SC7->C7_QTDREEM,2) + STR0034 + Str(nVias,2) + STR0035 ,nLinPC,1900 )
		Elseif(nRet == 2)
			oReport:PrintText( "1" + STR0034 + Str(nVias,2) + STR0035 ,nLinPC,1900 )
		Elseif(nRet == 3)
			oReport:CancelPrint()
		Endif
	Else
		oReport:PrintText( If( SC7->C7_QTDREEM > 0, Str(SC7->C7_QTDREEM+1,2) , "1" ) + STR0034 + Str(nVias,2) + STR0035 ,nLinPC,1900 )
	Endif
	...
	nLinPC := oReport:Row()
	oReport:PrintText(STR0087 + Alltrim(SM0->M0_NOMECOM),nLinPC,15)  // "Empresa:"
	//	oReport:PrintText(STR0106 + Alltrim(SA2->A2_NOME)+" - "+STR0107+SA2->A2_COD+" "+STR0108+SA2->A2_LOJA,nLinPC,1025)
	oReport:PrintText(STR0106 + Alltrim(SA2->A2_NOME),nLinPC,1025)
	oReport:PrintText(STR0107+SA2->A2_COD+" "+STR0108+SA2->A2_LOJA,nLinPC,1900)
	oReport:SkipLine()

	nLinPC := oReport:Row()
	oReport:PrintText(STR0088 + Alltrim(SM0->M0_ENDENT),nLinPC,15)
	//	oReport:PrintText(STR0088 + Alltrim(SA2->A2_END)+" - "+STR0109+SA2->A2_BAIRRO,nLinPC,1025)
	oReport:PrintText(STR0088 + Alltrim(SA2->A2_END),nLinPC,1025)
	oReport:PrintText(STR0109+SA2->A2_BAIRRO,nLinPC,1900)
	oReport:SkipLine()

	nLinPC := oReport:Row()
	oReport:PrintText(STR0089 + Trans(SM0->M0_CEPENT,PesqPict("SA2","A2_CEP"))+Space(2)+STR0090 + cCident + STR0091 + SM0->M0_ESTENT ,nLinPC,15)
	//	oReport:PrintText(STR0110+Alltrim(SA2->A2_MUN)+" - "+STR0111+SA2->A2_EST+" - "+STR0112+SA2->A2_CEP+" - "+STR0113+Transform(SA2->A2_CGC,PesqPict("SA2","A2_CGC")),nLinPC,1025)
	oReport:PrintText(STR0110+Alltrim(SA2->A2_MUN)+" - "+STR0111+SA2->A2_EST+" - "+STR0112+SA2->A2_CEP,nLinPC,1025)
	oReport:PrintText(STR0113+Transform(SA2->A2_CGC,PesqPict("SA2","A2_CGC")),nLinPC,1900)
	oReport:SkipLine()
	...
	oReport:PrintText(STR0094 + "("+Substr(SA2->A2_DDD,1,3)+") "+Substr(SA2->A2_TEL,1,15) + " "+STR0114+"("+Substr(SA2->A2_DDD,1,3)+") "+SubStr(SA2->A2_FAX,1,15),nLinPC,1025)
	oReport:PrintText(If( cPaisLoc$"ARG|POR|EUA",space(11) , STR0095 )+If( cPaisLoc$"ARG|POR|EUA",space(18), SA2->A2_INSCR ),nLinPC,1900)
	oReport:SkipLine()
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
/*/
User Function MATR110B(cAlias,nReg,nOpcx)
	...
	If nLastKey <> 27
		...
		If lAuto
			mv_par08 := Eval({|| cCont:=U_ChkPergUs(cUserId,"MTR110","08"),If(cCont == Nil,SC7->C7_TIPO,cCont) })
		EndIf
...
Static Function ImpCampos()
	...
	If mv_par08 == 1 .OR. mv_par08 == 3
		...
		@ li,114 PSAY "|"
		@ li,PCOL() PSAY Left(SC7->C7_CC,8) Picture PesqPict("SC7","C7_CC",18)
		@ li,135 - nDifColCC PSAY "|"
...
Static Function FinalPed(nDescProd)
	...
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	If Empty(MV_PAR13)
		@ li,003 PSAY STR0008 + Substr(SM0->M0_ENDENT,1,48)		//"Local de Entrega  : "
		@ li,072 PSAY "-"
		@ li,076 PSAY SM0->M0_CIDENT
		@ li,098 PSAY "-"
		@ li,100 PSAY SM0->M0_ESTENT
		@ li,103 PSAY "-"
	...
	li++
	@ li,001 PSAY "|"
	@ li,003 PSAY STR0010 + Substr(SM0->M0_ENDCOB,1,48)		//"Local de Cobranca : "
	@ li,072 PSAY "-"
	@ li,076 PSAY SM0->M0_CIDCOB
	@ li,098 PSAY "-"
	@ li,100 PSAY SM0->M0_ESTCOB
	@ li,103 PSAY "-"
	@ li,105 PSAY STR0009	//"CEP :"
	@ li,111 PSAY Trans(Alltrim(SM0->M0_CEPCOB),cCepPict)
	@ li,142 - nDifColCC PSAY "|"
	li++
	@ li,001 PSAY "|"
	@ li,002 PSAY Replicate("-",limite)
	...
	If !lNewAlc
		...
		@ li,001 PSAY "|"
		@ li,001 PSAY PADC(GETMV("MZ_MENSC7"),139)		//"|   NOTA: So aceitaremos a mercadoria se na sua Nota Fiscal constar o numero do nosso Pedido de Compras."
		@ li,142 - nDifColCC PSAY "|"
		li++
	...
	Else
		...
		@ li,101 PSAY STR0020		//"| Obs. do Frete: "
		@ li,119 PSAY IF( SC7->C7_TPFRETE $ "F","FOB",IF(SC7->C7_TPFRETE $ "C","CIF"," " ))
		@ li,141 - nDifColCC PSAY "|"
		li++
		...
		@ li,001 PSAY "|"
		@ li,005 PSAY PADR(GETMV("MZ_MENSC7"),100)		//"|   NOTA: So aceitaremos a mercadoria se na sua Nota Fiscal constar o numero do nosso Pedido de Compras."
		@ li,131 PSAY "|"
		li++

		/*If cEmpAnt+cFilAnt $ "0216"                     // Marcus Vinicius - 02/03/2017 - Ajuste para atender o chamado 24154 - (Aline)
			For AX:=1 TO 1
				@ li,001 PSAY "|"
				@ li,142 - nDifColCC PSAY "|"
				li++
			...
			@ li,001 PSAY "|"
			@ li,005 PSAY PADR("NÃO ACEITAMOS BOLETOS EM FACTORING",100)		//"|   NOTA: Nao aceitamos boleto em factoring
			@ li,131 PSAY "|"
			li++
		EndIf*/
		...
		if SX6->(DbSeek(cFilAnt+'MZ_MENSC7'))
			If !Empty(SX6->X6_CONTEUD)
				@ li,001 PSAY "|"
				@ li,005 PSAY PADR(SX6->X6_CONTEUD,100)
				@ li,131 PSAY "|"
				li++
			EndIf
			If !Empty(SX6->X6_CONTSPA)
				@ li,001 PSAY "|"
				@ li,005 PSAY PADR(SX6->X6_CONTSPA,100)
				@ li,131 PSAY "|"
				li++
			EndIf
			If !Empty(SX6->X6_CONTENG)
				@ li,001 PSAY "|"
				@ li,005 PSAY PADR(SX6->X6_CONTENG,100)
				@ li,131 PSAY "|"
				li++
			EndIf
		EndIf
