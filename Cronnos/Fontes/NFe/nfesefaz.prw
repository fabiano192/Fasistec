...
Local cChaveD2		:= ""
Local cChaveF2		:= ""
Local cTpOper		:= "" //Tipo de opera??o complemento
Local cChaveD1		:= "" 
...
Local lIpiDev   	:= GetNewPar("MV_IPIDEV",.F.)   	//Apenas para devolu??o de compra de IPI (nota de sa?da). T-S?ra gerado na tag vIPI e destacado no campo
														//VALOR IPI do cabe?alho do danfe. F-Ser? gerado na tag vIPIDevol e destacado nas informa??es complementares do danfe.
Local lIPIOutro 	:= GetNewPar("MV_IPIOUT",.F.) .And. lIpiDev //Quando habilitado juntamente com o MV_IPIDEV, o valor do IPI ser? gerado na tag vOutro, destacado nas informa??es complementares e no campo OUTRAS DESPESAS ACESSORIAS.
...
Local lEIPIOutro 	:= GetNewPar("MV_EIPIOUT",.F.) .And. lEipiDev //Quando habilitado juntamente com o MV_EIPIDEV, o valor do IPI ser? gerado na tag vOutro, destacado nas informa??es complementares e no campo OUTRAS DESPESAS ACESSORIAS.
...
Local nValOutr		:= 0
Local cTpOrig 		:= ""
Local lDescIss		:= SuperGetMV("MV_DESCISS",,.F.) //Informa ao sistema se o ISS devera ser descontado do valor do titulo financeiro caso o cliente for respons?vel pelo recolhimento.
Local lTpabiss		:= SuperGetMV("MV_TPABISS",,"2") == "1"//Se par?metro igual a 1 indica se ser? efetuado um desconto na duplicata quando o cliente recolhe ISS se igual a 2 ser? gerado um titulo de abatimento.
				...
				While !SF3->(Eof()) .And. cClieFor+cLoja+cNota+cSerie == SF3->F3_CLIEFOR+SF3->F3_LOJA+SF3->F3_NFISCAL+SF3->F3_SERIE
					...
					If nCont == 1
						Do While !SD2->(Eof ()) .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
								...
								EndIf
								//------------------------------------------------------------------------------------------------
								// Retirado o uso do parametro MV_SIMPREM para que a mensagem sej? gerado de acordo com o CSOSN
								//------------------------------------------------------------------------------------------------
								dbSelectArea("SF4")
								dbSetOrder(1)
								MsSeek(xFilial("SF4")+SD2->D2_TES)
			...
			Else
					...
					If !lUsaCliEnt
						...
						Else
							If !Empty(cCliNota+cCliLoja)
								SA1->(MsSeek(xFilial("SA1")+cCliNota+cCliLoja))   //Busca os dados do cliente da Nota sobre Cupom para montar os dados do destinat?rio do XML
							Else
								SA1->(MsSeek(xFilial("SA1")+SF2->F2_CLIENTE+SF2->F2_LOJA))
							EndIf
						EndIf
				...
				If SF2->F2_TIPO = 'N' .And. !SF2->F2_CLIENTE $ '000017|000018' .And. SA1->A1_TIPOCLI <> 'X'
					...
					*/
					...
					nScan := aScan(aEspVol,{|x| x[1] == cEspecie})
					If ( nScan==0 )
						aadd(aEspVol,{ cEspecie, SF2->F2_VOLUME1, SF2->F2_PLIQUI , SF2->F2_PBRUTO,xMarca,xNumero})
					Else
						aEspVol[nScan][2] += SF2->F2_VOLUME1
						aEspVol[nScan][3] += SF2->F2_PLIQUI
						aEspVol[nScan][4] += SF2->F2_PBRUTO
					EndIf
				Endif
				...
				If !Empty(SF2->F2_DUPL)	
					...
					#ELSE
						SE1->(MsSeek(xFilial("SE1")+SF2->F2_PREFIXO+SF2->F2_DOC))
					#ENDIF
					...
					//Verifica se existe Template DCL
	      			IF cVerAmb >= "4.00" .And. (ExistTemplate("PROCMSG")) //Tratativa para Grupo de Repasse de Combustiveis
						BeginSql Alias cAliasSD2
								...
								D2_QUANT,D2_TOTAL,D2_DESCON,D2_VALFRE,D2_SEGURO,D2_PEDIDO,D2_ITEMPV,D2_DESPESA,D2_VALBRUT,D2_VALISS,D2_PRUNIT,D2_PROCLI,D2_PEDCLI,
				...
				#ENDIF
							...
							_nVlOut		:= _nVlTot := xDespAce := _nMoeda := 0
							xPCM        := ""
							xAFM        := ""
							xAEM        := ""
	          ...
	          // Posiciono na TES do cupom fiscal para pegar a natureza de opera??o da nf sobre cupom
							   		...
							   		If MsSeek(xFilial("SFT")+"S"+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_NFORI)
										...
										Else
											dbSelectArea("SF3")
									   		...
									   		If MsSeek(xFilial("SF3")+SFT->FT_CLIEFOR+SFT->FT_LOJA+SFT->FT_NFISCAL+SFT->FT_SERIE)										   		
												//Obtem os dados de nota fiscal de produtor rural referenciada
												//A NF de sa?da Para este tipo de nota, o emitente eh sempre o sigamat.emp
                        ...
                        //Atualiza a Unid. Medida da DIPI(cUmDipi) e o Fator de Conv. da DIPI(nConvDip) com dados da SBZ caso os par?metro recebidos estejam vazios
						...
						*/
						...
						If SM0->M0_ESTENT $ "PR/RJ/RS/" //TAG cBenef buscar o conte?do da tabela 5.2 no sistema quando for do PR.
							...
							If MsSeek(xFilial("CDV") +'S'+PadR(cTpEspcBen,TamSX3("CDV_ESPECI")[1])+'S'+(cAliasSD2)->(D2_DOC+D2_SERIE+D2_CLIENTE+D2_LOJA+D2_ITEM))
								//Tratamento realizado enquanto o fiscal n?o realiza a cria??o do campo na CDV
						...
						EndIf
									If _lExport
										...
										If dbSeek(xFilial("EE7")+SC5->C5_PEDEXP)
											...
											If dbSeek(xFilial("EE8")+SC5->C5_PEDEXP + SC6->C6_ITEM )
												...
												/// TRECHO ABAIXO COMENTADO EM 19/10/10 DEVIDO GERAR ERRO NO MOMENTO DA TRANSMISS?O DA NFE
												/// QUANDO TEM QUE APARECER O VALOR DO FRETE NO DANFE (F2_FRETE)
										/*
										...
										_nMoeda := SM2->M2_MOEDA2
													Endif
										xFret    := Round(EE7->EE7_FRPCOM * _nMoeda,2)
										...
										*/
												...
												dbSelectArea("SY9")
												dbSetOrder(2)
												If dbSeek(xFilial("SY9")+EE7->EE7_ORIGEM )
													_cUFEM   := SY9->Y9_ESTADO
													_cMUNEM  := SY9->Y9_CIDADE
												Endif
									...
									Else
										...
										_nPos := AScan( _aPedido, SC5->C5_NUM)

										//							If _aPedido != SC5->C5_NUM
										If _nPos = 0
											xPESO_BRUTO += SC5->C5_PBRUTO           // Peso Bruto
											xPESO_LIQ   += SC5->C5_PESOL
											xVOLUME     += SC5->C5_VOLUME1
											AADD( _aPedido,SC5->C5_NUM)
										Endif
									...
									If _nMoeda > 0
										_nPrTot := ((xVALOR_MERC - _nVlOut) * (cAliasSD2)->D2_TOTAL) / xVALOR_MERC
										_nPrUni := _nPrTot / (cAliasSD2)->D2_QUANT
										_nVlTot += _nPrTot
									Else
										_nPrTot := (cAliasSD2)->D2_TOTAL
										_nPrUni := (cAliasSD2)->D2_PRCVEN
									Endif

									If !Empty(SC6->C6_CPROCLI)
										_cDesc := PadR(SC6->C6_CPROCLI,18)+": "+Alltrim(_cDESCSZ2)+" PED.CLIENTE: "+Alltrim(SC6->C6_PEDCLI)+" PED.CRONNOS+IT :"+SC6->C6_NUM+SC6->C6_ITEM
									Else
										_cDesc := Alltrim(_cDESCSZ2)+" - "+RTrim(SC6->C6_CPROCLI)+" PED.CLIENTE: "+Alltrim(SC6->C6_PEDCLI)+" PED.CRONNOS+IT :"+SC6->C6_NUM+SC6->C6_ITEM
									Endif

									_cPedCli   := Alltrim(SC6->C6_PEDCLI)
									_cItPedCli := Alltrim(SC6->C6_POLINE)
						aadd(aProd,	{Len(aProd)+1,;
							...
							0,;  //aprod[49]   nValLeite
									(cAliasSD2)->D2_PROCLI,;  // aprod[50] -- CNH
									(cAliasSD2)->D2_PEDCLI,;  // aprod[51] -- CNH
							})
						...
						aadd(aIPIDevol,{})
									...
									aadd(aPedcom,{SC6->C6_PEDCLI,SC6->C6_POLINE,SC6->C6_CLI})
						aadd(aPisAlqZ,{})
						aadd(aCofAlqZ,{})
						aadd(aCsosn,{})
									aadd(aInfPr,{})
						...
						If lEECFAT
							...
							Na posi??o 5 passa a ser enviado informa??oes para o grupo "BA02 - Chaves Nfe referenciadas" as chaves de notas fiscais de sa?da de lote de exporta??o associadas ? nota de sa?da de exporta??o.
						...
						While CD2->(!Eof()) .And. xFilial("CD2") == CD2->CD2_FILIAL .And.;
							...
							Do Case
								...
								Case AllTrim(CD2->CD2_IMP) == "ISS" 
									...
									//
									//Conjunto de Altera??es 229221 - Aline Yumi Kokumai - 21/05/2014 18:27:45 - Inclus?o do fonte para manter o hist?rico.

									A condi??o da valida??o do Codigo do ISS passou a ser Verdadeira (.T.) 
									devido as rotinas do REINF ter come?ado a popular a tabela CDN causando
									erro em municipios especificos.
									Portanto ser? mantido o legado da tabela CDN, por?m para emiss?o de NF-e
									conjugada ser? considerado o "C?digo de servi?o da Nota fiscal elet?nica" (SD2).
			    ...
			    EndDo 
						...
						nScan := aScan(aEspVol,{|x| x[1] == cEspecie})
																If ( nScan==0 )
							aadd(aEspVol,{ cEspecie, xVolume, xPESO_LIQ , xPESO_BRUTO,xMarca,xNumero})
																Else
							aEspVol[nScan][2] += xVolume
							aEspVol[nScan][3] += xPESO_LIQ
							aEspVol[nScan][4] += xPESO_BRUTO
																EndIf
															EndIf
			    ...
			    EndIf
															If _nValIcm >0
						cMensFis += "#ICMS DIFERIDO : "+Transform(_nValICM,"@E 999,999,999.99")
															Endif

															If !Empty(xPCM)
						cMensCli += "#Ped. : PCM "+Alltrim(xPCM)+" / AFM "+Alltrim(xAFM)+" / AEM "+Alltrim(xAEM)
															Endif
		...
		Else				
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				aAdd(aInfoItem,{(cAliasSD1)->D1_PEDIDO,(cAliasSD1)->D1_ITEMPC,(cAliasSD1)->D1_TES,(cAliasSD1)->D1_ITEM})
																	nScan2  := aScan(_aGrpMens2,SF4->F4_CODIGO)
																	If ( nScan2 == 0 )
																		AADD(_aGrpMens2,SF4->F4_CODIGO)
																	...
																	nScan2  := aScan(_aGrpMens3,SF4->F4_CODIGO)
																	If ( nScan2 == 0 )
																		AADD(_aGrpMens3,SF4->F4_CODIGO)
																		If Substr(SM0->M0_CGC,1,8) $ "00246279"
																			...
																			If !Empty(SF4->F4_FORMU04)
																				cMensFis += "#"+Alltrim(Formula(SF4->F4_FORMU04) )
																			EndIf
																		Endif
																	Endif

																	If SD1->(FieldPos("D1_MENNOTA")) <> 0
																		//					If !Empty((cAliasSD1)->D1_MENNOTA)
																		If !Empty(SD1->D1_MENNOTA)
																			//						cMensCli += AllTrim((cAliasSD1)->D1_MENNOTA)
																			cMensCli += AllTrim(SD1->D1_MENNOTA)
																		EndIf
																	Endif
				//Tratamento para que o valor de ICMS ST venha a compor o valor da tag vOutros quando for uma nota de Devolu??o, impedindo que seja gerada a rejei??o 610.
		       nIcmsST := 0
		       ...
		       EndIf   		
				...
				While !CD2->(Eof()) .And. xFilial("CD2") == CD2->CD2_FILIAL .And.;
					...
					If SubStr((cAliasSD1)->D1_CLASFIS,2,2) $ '51'	 .and. !Empty(SFT->FT_ICMSDIF)           .and. SFT->(ColumnPos("FT_VOPDIF")) > 0  .and. !Empty(SFT->FT_VOPDIF) .or.; // verifica diferimento com bloqueio de movimento
						...
						lDifer :=.T.
					Else
						lDifer := .F. //Reinicializa??o da vari?vel
					EndIf					
...
If !Empty(aIEST) 
	...
	*/
	/* Adicionado tratativa para destacar em XML I.E. especial, para cliente ST, em opera??es internas no estado do PR, conforme Art. 176 ? 10 do RICMS 2017 */                                                                                                                                                                                                                                                       
...
//Valida??o para controle das tags que dever?o ser preenchidas apenas nas opera??es?n?o destinadas a consumidor final RS
...
If (aCST[01] = "60" .or. cCsosn$ "500") .and. !Empty(cUltAqui)  .and. Len(aIcms)>19
	//novos campos na tabela SFT s?o: FT_BSTANT (Base) FT_PSTANT (Percentual) FT_VSTANT (Valor) Atenciosamente.
	nBaseIcm :=  aICMS[20]      //SFT->FT_BSTANT
	nValICM  :=  aICMS[21]      //SFT->FT_VSTANT
	nAlqICM  :=  aICMS[22]      //SFT->FT_PSTANT
	//novos campos do FECP na tabela SFT s?o: FT_BFCANTS (Base) FT_PFCANTS (Percentual) FT_VFCANTS (Valor) Atenciosamente.
...
If  !lIssQn    
	...
	ELse
		...
		If (aCST[01] = "60" .or. cCsosn$ "500")		
			//Se o par?metro  MV_ULTAQUI que trata a ?ltima aquisi??o n?o estiver preenchido usa o modelo antigo de rastro.
			If Empty(cUltAqui) 
				SPEDRastro2(aProd[20],aProd[19],aProd[02],@nBaseIcm,@nValICM,,,lCalcMed,@nAlqICM,,,,,,,,,,,@nBfcpant,@nAfcpant,@nVfcpant)
			...
			If nBaseIcm > 0 .and. nValICM > 0	.and. nAlqICM > 0
			    ...
			    If cArt274 == "1"  .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) == "SP"
					...
					if Empty(At("Artigo 274 do RICMS", cMensFis))
						cMensFis += "Imposto Recolhido por Substitui??o - Artigo 274 do RICMS (Lei 6.374/89, art.67,Paragrafo 1o., e Ajuste SINIEF-4/93',cl?usa terceira, na reda??o do Ajuste SINIEF-1/94) &|"+ConvType(aProd[02])+"|"+AllTrim(str(nValICM,15,2))+"|&"
					else
				...
				ElseIf cArt274 == "1"  .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) == "ES" .And. Len(aICMS) > 0 .And. ( nBaseIcm+nValICM > 0 )
					...
					cMensFis += "ICMS da opera??o pr?pria do contribuinte substituto - R$ "+Alltrim(str(aIcms[7],15,2))+". "
				...
				ElseIf IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) == "SP" 
					If cIndFinal == "1"
						cMensFis += "Imposto Recolhido por Substitui??o - Contempla o artigo 313-Z19 do RICMS-SP."
					Else
				...
				ElseIf IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) == "RJ"
					if aComb[01] $ NfeProdANP()
						...
						cMensFis += "ICMS a ser repassado nos termos do Cap?tulo V do Conv?nio ICMS 110/07. Valor da Base de ST: R$ "+Alltrim(str(nBaseIcm,15,2))+" Valor de ICMS ST: R$ "+Alltrim(str(nValICM,15,2))+" "
					elseif cArt274 == "1"
	...
	If (cIdDest == "2" .and. cIndFinal == "1" .and. cIndIEDest == "9") .and. (Len(aISSQN)== 0) .and. ;
	   ...
	   (iif(Len(aComb) > 0 .And. !Empty(aComb[01]),aComb[01] $ NfeCodANP(),.T.))
		...
		If Len(aICMUFDest) > 0 .And. !(aNota[04] == "0" .and. aNota[5] == "N" .And. aICMUFDest[05] == 0)
			...
			// <AliquotaInter> padr?o 4,7 ou 12. Em caso de pedido com produtos com aliquotas diferentes, as al?quotas ser?o separadas em v?rgula para exibi??o. (Ex.: 4.00%, 7.00%)
...
EndIf
// ALTERADO EM 17/03/20

If Len(aPedCom) = 3
	If aPedCom[03] == "000051" // CNH
		// cString += '<infadprod>IAP01'+PADR(aProd[50],30)+Alltrim(aProd[51])+'</infadprod>'
		cString += '<infadprod>IAP01'+PADR(PadL(Alltrim(aProd[50]),11,"0"),30)+PadL(Alltrim(aProd[51]),12,"0")+'</infadprod>'
	Else
...
If cVerAmb == "4.00"
	//Se opera??o interestadual(idDest=2), n?o informar os Grupos Veiculo Transporte (id:X18; veicTransp) e Grupo Reboque (id: X22)
	//Obs1: a crit?rio de cada UF, a regra de valida??o acima tamb?m pode ser aplicada nas opera??es internas (idDest=1) se cMun (id:C10) do Emitente <> cMun (id: E10) do Destinat?rio
			...
			Else
				if SM0->M0_ESTENT == 'SP'
					...
					if (nPos := aScan(aDadosDest,{|x| x[1]+x[2]+x[10] == aNfVinc[nZ][12]+aNfVinc[nZ][13]+AllTrim(Str(aNfVinc[nZ][11])) }) ) == 0
						...
						cString += alltrim(aDadosDest[nPos,6]) + ', ' + alltrim(aDadosDest[nPos,7]) + ', inscrita no CNPJ sob n? ' + alltrim(aDadosDest[nPos,8]) + ' e Inscricao Estadual '
						cString += alltrim(aDadosDest[nPos,9]) + ' atraves das NFs: '
						lMsnOk:= .T.
					EndIf
					cString += "NF-e: "
				Else
					cString += "Emiss?o Original NF-e: "
				endIf
			endif
...
If nTotCrdP > 0 // valor de cr?dito Presumido -  Art.75, XXXII do RICMS/MG
	cString += "Valor de cr?dito Presumido: R$ " + AllTrim(Transform(nTotCrdP, "@ze 9,999,999,999,999.99")) + ". "
EndIf
...
If nvFCPUFDest > 0 .or.  nvICMSUFDest > 0 .or. nvICMSUFRemet > 0 .or. nvBCUFDest  > 0     
    ...
    cString +="Valor da BC do ICMS na UF de destino: R$ "+ConvType(nvBCUFDest,15,2)+". "  
 		...
 		cString +="Al?quota interestadual das UF envolvidas: "+cMensDifal+ ". "
...
Retorna a Unid. Medida da DIPI e o Fator de Conv. da DIPI da SBZ caso os par?metro recebidos estejam vazios.

@param  cProduto - Produto que ser? localizado na SBZ
...
Valida??o com a tabela 5.2 para enviar o codigo SEM CBENEF ou NULO de acordo com a UF

@param  cUF 	- Estado que ser? validado
...
Limpa os c?digos ASCII faixa 127 a 255 que falham os EncodeUTF8
...
Verifica se a opera??o se trata de uma triangula??o
...
Fun??o?para?que?quando?mudar?o?cadastro?de?cliente referente ao grupo de destinat?rio?na?tabela?AIF?o mesmo?busque?dados?da?nota?vinculada e retorna no array .?
@param  aDest 	 - Array com o destinat?rio
	...
	SELECT max(AIF_DATA) AIF_DATA
		...
		if lDestVinc
			...
			If !Empty(aDestVinc)
				...
				aDest[04]  := aDestVinc[04] // - N?mero
				aDest[05]  := aDestVinc[05] // - Complemento
				aDest[06]  := aDestVinc[06] // - Bairro
				aDest[07]  := aDestVinc[07] // - C?digo do munic?pio
				aDest[08]  := aDestVinc[08] // - Nome do munic?pio
				aDest[09]  := aDestVinc[09] // - Sigla da UF
				aDest[10]  := aDestVinc[10] // - C?digo do CEP
				aDest[11]  := aDestVinc[11] // - C?digo do Pa?s
				aDest[12]  := aDestVinc[12] // - Nome do Pa?s
				aDest[13]  := aDestVinc[13] // - Telefone
				aDest[14]  := iif(EMPTY(aDestVinc[15]),'ISENTO',aDestVinc[15]) // - Inscri??o Estadual do Destinat?rio
				aDest[15]  := aDestVinc[16] // - Inscri??o na SUFRAMA
				aDest[19]  := aDestVinc[17] // - Inscri??o Municipal do Tomador do Servi?o
