...
Data 		³ 	05.02.14
Descricao 	³ 	Tela de Chamados de TI
/*/
...
User Function CR0054()

	Private	_cEmpUso  	:= AllTrim(cEmpAnt)+"/"
	Private	_cPerg    	:= "CR0054"
	Private	_bFiltraBrw	:= ''
	Private	_aIndexSZ3 	:= {}
	Private	_cFiltro  	:= ''
	Private	_aAnexo		:= {}
	Private	_nAnexo		:= 0
	Private cDir   		:= '/Chamados_TI/'
...
//		EndIf
	...
	If	( cNivel < 8 )
		If	( nOpcao == 4 ) .or. ( nOpcao == 5 ) .or. ( nOpcao == 6 )
			Help('',1,'CR0054',,OemToAnsi('Você não tem permissão para '+cTitWin+'.'),1)
			Return
		EndIf
	EndIf
	...
	If	( nOpcao==4 ) .and. ( ! Empty(SZ3->Z3_TECNICO) )
		Help('',1,'CR0054',,OemToAnsi('Este chamado já foi encerrado.'),1)
		Return
	EndIf
	...
	DEFINE MSDIALOG oWindow FROM 38,16 TO 550,531 TITLE Alltrim(OemToAnsi('Chamados - Ocorrências ['+cTitWin+']')) Pixel //430,531
	...
	@ 017, 040 MsGet cPriCham Size 035, 010 F3 'ZP ' Picture('@!') Valid xValid(2) When nOpcao<>5.and.nOpcao<>2 Of oFolder:aDialogs[1] Pixel
	@ 017, 083 Say _oPriCham Var _cPriCham Font oFontWin Of oFolder:aDialogs[1] Pixel
		
	@ 029, 005 Say _oModulo Var OemToAnsi('Módulo:') Size 025, 007 Of oFolder:aDialogs[1] Pixel
	@ 029, 040 MsGet _oModCha1 Var cModCham Size 035, 010 F3 'ZN' Picture('@!') Valid xValid(8) When nOpcao<>5.and.nOpcao<>2 Of oFolder:aDialogs[1] Pixel
	...
	@ 041, 040 MsGet _oProgCham Var cProgCham Size 060, 010 Picture('@!') When nOpcao<>5.and.nOpcao<>2 Of oFolder:aDialogs[1] Pixel
	If	( Empty(dDtFecha) )
	...
	@ 041, 005 Say OemToAnsi('Ocorrência:') Size 028, 007 Of oFolder:aDialogs[2] Pixel
	@ 041, 040 Get oAberOcor Var mAberOcor Memo When nOpcao<>5.and.nOpcao<>2 Size 200, 100 Of oFolder:aDialogs[2] Pixel
	oAberOcor:oFont:=oFontMemo
	
	@ 145,005 BUTTON "ANEXO(s)" SIZE 030, 010 OF oFolder:aDialogs[2] PIXEL ACTION Anexo(oFolder:aDialogs[2])
				
		//ÚÄÄÄÄÄÄÄÄÄÄÄ¿
		//³Comentarios³
		//ÀÄÄÄÄÄÄÄÄÄÄÄÙ
	If	( nOpcao == 2 ) .or. ( nOpcao == 4 ) .or. ( nOpcao == 5 ) .or. ( nOpcao == 6 )
		@ 005, 005 Say OemToAnsi('Téc.Alocado:') Size 030, 007 Of oFolder:aDialogs[3] Pixel
		...
		EndIf
		@ 017, 005 Say OemToAnsi('Comentários:') Size 030, 007 Of oFolder:aDialogs[3] Pixel
	...
	If	( nOpcao == 2 ) .or. ( nOpcao == 5 ) .or. ( nOpcao == 6 )
		...
		@ 017, 005 Say OemToAnsi('Técnico:') Size 025, 007 Of oFolder:aDialogs[4] Pixel
		...
		@ 041, 005 Say OemToAnsi('Solução:') Size 028, 007 Of oFolder:aDialogs[4] Pixel
		...
		@ 145, 040 Radio oRadio Var nRadio When lEdite.and.nOpcao<>5.and.nOpcao<>2 Size 090,007 Of oFolder:aDialogs[4] Pixel Prompt OemToAnsi('Não enviar'),OemToAnsi('E-Mail do Solicitante')
		@ 166, 040 Say OemToAnsi('Cópia E-mail:') Size 030, 007 Of oFolder:aDialogs[4] Pixel
	...
	If ( lSave )
		...
		If	( nOpcao <> 2 )
			...
			If	!Empty(_cSubMail)
				MsgRun(OemToAnsi('Enviando e-mail.... Aguarde....'),'',{|| CursorWait(), WfChamado(_cSubMail) ,CursorArrow()})
			Endif
	...
	Else
		...
		For nI:=1 to Len(aFiles)
			If Left(aFiles[nI][1],8) = cNumCham
//				Aadd( aListFile, {.F., aFiles[nI][1]} )
				fErase(cDir+(aFiles[nI][1]))
			Endif
		Next nI
...
User Function CHAMCan()
	...
	If	( SZ3->Z3_STATUS == '0' )
		...
		EndIf
	Else
		Help('',1,'CR0054',,OemToAnsi('Este chamado não pode ser cancelado.'),1)
	EndIf
...
Static Function WfAvisaCham( cTo,_cCopia )
	...
	Local	_cSmtpSrv 	:= "201.16.208.157",;
		_cAccount 	:= "workflow@cronnos.com.br",;
		_cPassSmtp	:= "workflow",;
		_cSmtpError	:= '',;
		_lOk		:= .f.,;
		_cTitulo 	:= OemToAnsi('[CRONNOS] Encerramento Chamado: '+cNumCham),;
		_cTo		:= cTo,;
		_cFrom		:= _cMailTec,;
		_cMensagem	:= '',;
		_lReturn	:= .f.
	...
	_cMensagem	+= OemToAnsi('Solução: ') + mSoluOcor + CHR_ENTER + CHR_ENTER
	_cMensagem	+= OemToAnsi('Técnico: ') + _cTecCham + CHR_ENTER + CHR_ENTER
	_cMensagem	+= CHR_LINE + CHR_ENTER + CHR_NEGRITO
	_cMensagem	+= OemToAnsi('Cronnos Ind. e Com. de Artefatos Elastoméricos Ltda.') + CHR_NOTNEGRITO + CHR_ENTER
	_cMensagem	+= CHR_NEGRITO + OemToAnsi('Tecnologia da Informação') + CHR_ENTER + CHR_NOTNEGRITO
...
//Esta funcao envia um e-mail para os responsáveis.
Static Function WfChamado(_cSubMail)

	Private _lRet

	nOpcao := 0

	CONOUT("Abertura Chamado TI")

	oProcess := TWFProcess():New( "CHAMADO", "TI" )
	oProcess:NewTask( "CHAMADO", "\WORKFLOW\CR0054.htm" )
	oHTML := oProcess:oHTML

	oHtml:ValByName("chamado"		, cNumCham)
	oHtml:ValByName("data"			, DTOC(dDtAbert))
	oHtml:ValByName("hora"			, dHrAbert)
	oHtml:ValByName("solic"			, _cSolCham)
	oHtml:ValByName("descric"		, cDesCham)
	oHtml:ValByName("tipo"			, _cTipCham)
	oHtml:ValByName("priori"		, _cPriCham)
	oHtml:ValByName("modulo"		, _cModCham)
	oHtml:ValByName("programa"		, cProgCham)
	oHtml:ValByName("ocorr"			, mAberOcor)
                         

	oProcess:cSubject := _cSubMail+" Chamado TI - "+Dtoc(dDataBase)+" Hora : "+Substr(Time(),1,5)

	CONOUT(_cSubMail+" Chamado TI - "+Dtoc(dDataBase)+" Hora : "+Substr(Time(),1,5))

	oProcess:fDesc := "Chamado TI"
...
//	_cCC	+= USRRETMAIL(RETCODUSR())
	_cCC	+= ';'+USRRETMAIL(cSolCham)

	aFiles	:= Directory( cDir + '*.*' )
	
	For nI:=1 to Len(aFiles)
		If Left(aFiles[nI][1],8) = cNumCham
			oProcess:AttachFile(cDir+(aFiles[nI][1]))
			If _cSubMail == 'Exclusão"
				fErase(cDir+aFiles[nI][1])
			Endif
		Endif
	Next nI
...
User Function CHAMPRINT()
	...
	Private	cStartPath	:= GetSrvProfString('Startpath',''),;
		cFileLogo	:= cStartPath + 'logCr.bmp'
...
Static Function xCabecPrint()
	...
	oPrint:Say(nLin+20,700,'Cronnos Ind. e Com. de Artefatos Elastoméricos Ltda.',oFont16)
	oPrint:Say(nLin+150,700,'Tecnologia da Informação',oFont11)
...
Static Function AjustaSx1(cPerg)
	...
	aAdd(_aHelp[02],OemToAnsi("De qual prioridade você quer visualizar?"))
...
Static Function ANEXO()
	...
	DEFINE MSDIALOG oDlg TITLE "Anexos" FROM 0,00 TO 300,400 OF oMainWnd PIXEL

	@ 010,10 LISTBOX  oListArq VAR cList FIELDS HEADER "", "Anexo" SIZE  180, 120 OF oDlg ON DBLCLICK ( MudaMarca() ) PIXEL

	oListArq:SetArray( aListFile )
	oListArq:bLine	:= { || { If( aListFile[oListArq:nAt,1],oOk,oNo ),aListFile[oListArq:nAt,2]} }

	If	nOpcao = 3  .Or. nOpcao = 4
		@ 135,025 BUTTON "Incluir" 	SIZE 030, 010 OF oDlg PIXEL ACTION (nOpc := 1,oDlg:End())
		@ 135,075 BUTTON "Excluir" 	SIZE 030, 010 OF oDlg PIXEL ACTION (nOpc := 2,oDlg:End())
	Endif
	@ 135,125 BUTTON "Sair" 	SIZE 030, 010 OF oDlg PIXEL ACTION oDlg:End()

	ACTIVATE MSDIALOG oDlg CENTERED

	If nOpc == 1

		_cFile := cGetFile('Arquivo *|*.*|Arquivo DOCX|*.docx|Arquivo XLSX|*.xlsx|Arquivo PDF|*.pdf','Selecione arquivo',0,'C:\',.F.,GETF_NETWORKDRIVE+GETF_LOCALHARD,.F.)
	
		If !Empty(_cFile)
			_cFil1 := cNumCham +'_'+Right(_cFile,Len(_cFile)-3)

			If !__CopyFile(_cFile, "\Chamados_TI\"+_cFil1 )
				MsgInfo('Não foi possível anexar o arquivo: '+_cFile)
			Else
				AADD(aListFile,_cFil1)
//				AADD(_aAnexo,_cFil1)
			Endif
		Endif
	...
	ElseIf nOpc == 2
		For E:= 1 To Len(aListFile)
			If aListFile[E][1]
				fErase(cDir+aListFile[E][2])
			Endif
		Next E
...
//Muda a marca de selecao do item da ListBox.
Static Function MudaMarca()

	aListFile[ oListArq:nAt, 1 ] := !aListFile[ oListArq:nAt, 1 ]

	For nInd:=1 To Len(aListFile)

		If nInd <> oListArq:nAt
			If aListFile[nInd][1]
				aListFile[nInd][1] := .F.
			Endif
		Endif
