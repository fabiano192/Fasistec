...
SELECT TIPO = 'PONTUALIDADE',SUM(TAB.OK) AS TOK,SUM(TAB.NOK) AS TNOK,SUM(TAB.TOT) AS TOTAL,
TAB.INDICE ,
(RIGHT(TAB.DTENTDE,2)+'/'+SUBSTRING(TAB.DTENTDE,5,2)+'/'+LEFT(TAB.DTENTDE,4)) AS DTENTDE,
(RIGHT(TAB.DTENTAT,2)+'/'+SUBSTRING(TAB.DTENTAT,5,2)+'/'+LEFT(TAB.DTENTAT,4)) AS DTENTAT
FROM (
SELECT DISTINCT A.FORNECE,A.LOJA,A.NOME,A.DTENTDE,A.DTENTAT,A.INDICE,
OK = (SELECT COUNT(*) FROM CR0078 B 
		WHERE A.FORNECE = B.FORNECE AND A.LOJA = B.LOJA
		AND B.DTENTNF <= (B.DTENTPC+1) AND B.QTDEPC = B.QTDENF),
NOK = (SELECT COUNT(*) FROM CR0078 C 
		WHERE C.FORNECE = A.FORNECE AND C.LOJA = A.LOJA
		AND ((C.DTENTNF > (C.DTENTPC+1)) OR (C.QTDEPC <> C.QTDENF) )),
TOT = (SELECT COUNT(*) FROM CR0078 C 
		WHERE C.FORNECE = A.FORNECE AND C.LOJA = A.LOJA)
 FROM CR0078 A) TAB
GROUP BY TAB.INDICE,TAB.DTENTDE,TAB.DTENTAT




select FORNECE,LOJA,NOME,PRODUTO,UM,PEDIDO,ITEMPC,
(RIGHT(DTENTPC,2)+'/'+SUBSTRING(DTENTPC,5,2)+'/'+LEFT(DTENTPC,4)) AS DTENTPC,
(RIGHT(DTENTNF,2)+'/'+SUBSTRING(DTENTNF,5,2)+'/'+LEFT(DTENTNF,4)) AS DTENTNF,
(RIGHT(DTENTPC,2)+'/'+SUBSTRING(DTENTPC,5,2)+'/'+LEFT(DTENTPC,4)) AS DTEMIPC,
QTDEPC,QTDENF,NF,SERIE
FROM CR0078
