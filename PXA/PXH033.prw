...
//³                                                                        ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
oReport:= TReport():New("PXH033",STR0001,"MTR080", {|oReport| ReportPrint(oReport,cAliasSD1)},STR0002+" "+STR0003+" "+STR0004) //"Rela‡„o de Notas Fiscais"##"Emiss„o da Rela‡„o de Notas de Compras"##"A op‡„o de filtragem deste relat¢rio s¢ ‚ v lida na op‡„o que lista os"##"itens. Os parƒmetros funcionam normalmente em qualquer op‡„o."
...
oSection1 := TRSection():New(oReport,STR0073,{"SD1","SF1","SA1","SA2","SB1","SE2"},aOrdem) //"Rela‡„o de Notas Fiscais"
oSection1 :SetTotalInLine(.F.)

//--------- Celulas utilizadas para ordem de Nota --------
TRCell():New(oSection1,"_cEmp"     ,"  " ,"Empresa"    ,"@!",05,/*Picture*/,/*Tamanho*/,/*lPixel*/,/*{|| code-block de impressao }*/)  
TRCell():New(oSection1,"_cFil"     ," "  ,"Filial"     ,"@!",02,/*Picture*/,/*Tamanho*/,/*lPixel*/,/*{|| code-block de impressao }*/)  
TRCell():New(oSection1,"_cNomFil"  ," "  ,"Nome Filial","@!",15,/*Picture*/,/*Tamanho*/,/*lPixel*/,/*{|| code-block de impressao }*/) 
...
TRCell():New(oSection1,"E2_VALOR"  ,"SE2",/*Titulo*/,/*Picture*/,/*Tamanho*/,/*lPixel*/,/*{|| code-block de impressao }*/,,,,,,.F.)
...
TRCell():New(oSection1,"E2_YPRAPG" ,"SE2",/*Titulo*/,/*Picture*/,/*Tamanho*/,/*lPixel*/,/*{|| code-block de impressao }*/) 
TRCell():New(oSection1,"E2_YFORMPG","SE2",/*Titulo*/,/*Picture*/,/*Tamanho*/,/*lPixel*/,/*{|| code-block de impressao }*/) 
TRCell():New(oSection1,"E2_YFORMPG","SE2",/*Titulo*/,/*Picture*/,/*Tamanho*/,/*lPixel*/,/*{|| code-block de impressao }*/)  
TRCell():New(oSection1,"F1_COND"   ,"SF1",/*Titulo*/,/*Picture*/,/*Tamanho*/,/*lPixel*/,/*{|| code-block de impressao }*/)
TRCell():New(oSection1,"E2_VENCREA","SE2",/*Titulo*/,/*Picture*/,/*Tamanho*/,/*lPixel*/,/*{|| code-block de impressao }*/)
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
oSection2 := TRSection():New(oSection1,STR0074,{"SD1","SF1","SA1","SA2","SB1","SE2"})
oSection2 :SetTotalInLine(.F.)

//--------- Celulas utilizadas para ordem de Nota --------
TRCell():New(oSection2,"_cEmp","   ","Empresa","@!",05,/*lPixel*/,/*{|| code-block de impressao }*/)
TRCell():New(oSection2,"_cFil","   " ,"Filial"    ,"@!",02,/*lPixel*/,/*{|| code-block de impressao }*/) 
TRCell():New(oSection2,"_dVencto"," ","Vencimento",""  ,,/*lPixel*/,/*{|| code-block de impressao }*/)
...
oSection1:Cell("_cEmp"):SetValue(cEmpAnt+Left(cFilAnt,3)) 
oSection1:Cell("_cFil"):SetValue(Right(cFilAnt,2))
...
_cEmp   := cEmpant+Left(cFilAnt,3)
_cFil   := Right(cFilAnt,2)
...
While !oReport:Cancel() .And. (cAliasSD1)->(!Eof()) .And. (cAliasSD1)->D1_FILIAL == xFilial("SD1")
	...
	If lFiltro
		...
		SE2->(dbSetOrder(6))
		If SE2->(dbseek(xFilial("SE2")+(cAliasSF1)->F1_FORNECE +(cAliasSF1)->F1_LOJA + (cAliasSF1)->F1_SERIE + (cAliasSF1)->F1_DOC))
			_dVencto := SE2->E2_VENCREA
		Endif
		...
		If ((cAliasSD1)->D1_FILIAL + (cAliasSD1)->D1_DOC + (cAliasSD1)->D1_SERIE + (cAliasSD1)->D1_FORNECE + (cAliasSD1)->D1_LOJA <> cDocAnt)
			...
			If (nValIcm + nValMerc + nValIpi + nValImpInc + nValImpNoInc) > 0
				...
				ElseIf mv_par05 == 2
					...
					//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
					oSection1:Cell("_cEmp"):SetValue(_cEmp) 
					oSection1:Cell("_cFil"):SetValue(_cFil)
					...
					oSection1:Cell("_dVencto"):SetValue(_dVencto)
...
If nOrdem == 1 .And. mv_par05 == 1
	...
	cCabec   := STR0012	//"CODIGO DO MATERIAL   D E S C R I C A O             CFO  TE  ICM  IPI  LOCAL             QUANTIDADE   VALOR UNITARIO      VALOR TOTAL"
Elseif nOrdem == 1 .And. mv_par05 == 2
	...
	Else                                                       //"NUMERO       SER   DATA     CODIGO RAZAO SOCIAL                       PRACA       PGTO     ICM            IPI       TOTAL MERCADORIA"
		cabec1   := STR0033  // 123456789012 123 99/99/9999 123456 123456789012345678901234567890 123456789012345 XXX 999,999,999.99 999,999,999.99 9,999,999,999.99
	Endif
Elseif nOrdem == 2
	nPosNome := 29
	nTamNome := 30
	...
	If aTamSXG[1] != aTamSXG[3]			                     //"NUMERO       EMISSAO    FORNEC               RAZAO SOCIAL    LC CFO TE  ICM   IPI   TP    QUANTIDADE VALOR UNITARIO     VALOR TOTAL"
		cCabec  := STR0048     // 012345678901 99/99/9999 12345678901234567890 1234567890123456 12 123 123 12345 12345 1  1234567890123 12345678901234  12345678901234
	Else
...
Elseif (nOrdem == 3 .Or. nOrdem == 4) .And. mv_par05 == 1
	...
	If aTamSXG[1] != aTamSXG[3]                             //"NUMERO       PRODUTO         FORNEC              RAZAO SOCIAL LC CFO TE  ICM   IPI   TP    QUANTIDADE VALOR UNITARIO      VALOR TOTAL"
		cCabec  := STR0049       // 123456789012 123456789012345 12345678901234567890 12345678901 12 123 001 12345 12345 1  9,999,999,999 99,999,999,999 9999,999,999,999
...
Elseif nOrdem == 5 .And. mv_par05 == 1
	...
	cCabec := STR0063	 // 123456789012 123456789012345   1234567890     1234567890      12 123 001 12345 12345 1  9,999,999,999 99,999,999,999 9999,999,999,999
Endif
