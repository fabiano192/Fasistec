...
#DEFINE HMARGEM   030
#DEFINE VMARGEM   030
#DEFINE MAXITEM   022 //022                                                // Máximo de produtos para a primeira página
#DEFINE MAXITEMP2 046 //068                                                // Máximo de produtos para a pagina 2 (caso nao utilize a opção de impressao em verso)
...
//If SC5->C5_TPFRETE=="C"   // ALTERADO POR SERGIO EM 18/08
If SC5->C5_TPFRETE$"C/S"
	cModFrete := "0"
...
For nZ := 1 To nLenDet
	...
	nMaxDes := MaxCod(NoChar(oDet[nX]:_Prod:_xProd:TEXT,lConverte), 100)
...
If Type("oNF:_InfNfe:_infAdic:_infAdFisco:TEXT")<>"U"
	cAux2 := oNF:_InfNfe:_infAdic:_InfAdFisco:TEXT
	aAux  := STRTOKARR(cAux2,"#")
	
	For nX := 1 To Len(aAux)
		cAux := aAux[nX]
		While !Empty(cAux)
			
			aadd(aMensagem,SubStr(cAux,1,105))
			
			cAux := SubStr(cAux,106)
		EndDo
	Next nX
EndIf
...
If Type("oNF:_InfNfe:_infAdic:_infCpl:TEXT")<>"U"
	cAux2 := oNF:_InfNfe:_infAdic:_InfCpl:TEXT
	aAux := STRTOKARR(cAux2,"#")
	For nX := 1 To Len(aAux)
		cAux := aAux[nX]
		While !Empty(cAux)
...
While nLenItens>0                                                           	
	...
	If lPagPar
		nSoma := nItensRest - Iif( MV_PAR05==1 , 26 , 69 )
...
nForTo		+=	Iif(nForTo>Round(nForTo,0),Round(nForTo,0)+1-nForTo,nForTo)
For nX := 1 To nForTo
	oDanfe:Say(nLinCalc,098,SubStr(cStrAux,Iif(nX==1,1,((nX-1)*35)+1),35),oFont08N:oFont)
	nLinCalc+=10
Next nX
...
If nfolha==1
	If lMv_Logod
		oDanfe:SayBitmap(042,000,cLogoD,095,056)
	Else
		oDanfe:SayBitmap(042,000,cLogo,095,056)
	EndIF
Endif
...
oDanfe:Say(158,002,oIdent:_NATOP:TEXT+'-'+alltrim(tabela('13',oIdent:_NATOP:TEXT)),oFont08:oFont)
...
oDanfe:Say(450,054,"DESCRIÇÃO DO PROD./SERV.",oFont08N:oFont)
...
For nY := 1 To nLenItens
	...
	If lflag
		If nL > MAXITEM
			...
			oDanfe:Box(163+nLinhavers,000,nColLim,052)
			oDanfe:Say(171+nLinhavers,002,"COD. PROD",oFont08N:oFont)
			
			oDanfe:Box(163+nLinhavers,052,nColLim,155)
			oDanfe:Say(171+nLinhavers,054,"DESCRIÇÃO DO PROD./SER.",oFont08N:oFont)
			
			oDanfe:Box(163+nLinhavers,155,nColLim,190)
			oDanfe:Say(171+nLinhavers,157,"NCM/SH",oFont08N:oFont)
			
			oDanfe:Box(163+nLinhavers,190,nColLim,210)
			oDanfe:Say(171+nLinhavers,192,"CST",oFont08N:oFont)
			
			oDanfe:Box(163+nLinhavers,210,nColLim,230)
			oDanfe:Say(171+nLinhavers,212,"CFOP",oFont08N:oFont)
			
			oDanfe:Box(163+nLinhavers,230,nColLim,250)
			oDanfe:Say(171+nLinhavers,232,"UN",oFont08N:oFont)
			
			oDanfe:Box(163+nLinhavers,250,nColLim,289)
			oDanfe:Say(171+nLinhavers,252,"QUANT.",oFont08N:oFont)
			
			oDanfe:Box(163+nLinhavers,289,nColLim,346)
			oDanfe:Say(171+nLinhavers,292,"V.UNITARIO",oFont08N:oFont)
			
			oDanfe:Box(163+nLinhavers,346,nColLim,400)
			oDanfe:Say(171+nLinhavers,348,"V.TOTAL",oFont08N:oFont)
			
			oDanfe:Box(163+nLinhavers,400,nColLim,465)
			oDanfe:Say(171+nLinhavers,402,"BC.ICMS",oFont08N:oFont)
			
			oDanfe:Box(163+nLinhavers,465,nColLim,515)
			oDanfe:Say(171+nLinhavers,467,"V.ICMS",oFont08N:oFont)
			
			oDanfe:Box(163+nLinhavers,515,nColLim,550)
			oDanfe:Say(171+nLinhavers,517,"V.IPI",oFont08N:oFont)
			
			oDanfe:Box(163+nLinhavers,550,nColLim,578)
			oDanfe:Say(171+nLinhavers,552,"A.ICMS",oFont08N:oFont)
			
			oDanfe:Box(163+nLinhavers,578,nColLim,603)
			oDanfe:Say(171+nLinhavers,582,"A.IPI",oFont08N:oFont)
	...
	If	nL > Iif( (nfolha-1)%2==0 .And. MV_PAR05==1,MAXITEMP3,MAXITEMP2)
		oDanfe:EndPage()
		oDanfe:StartPage()
		nLenMensagens:= Len(aMensagem)							
		...
		oDanfe:Say(171,062,"DESCRIÇÃO DO PROD./SER.",oFont10N:oFont)
		...
		*/
		...
		oDanfe:Say(171,054,"DESCRIÇÃO DO PROD./SER.",oFont08N:oFont)
