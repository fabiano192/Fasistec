		...
		Else
			...
			#IFDEF TOP
				...
				BeginSql Alias cAliasSD2
						...
						D2_ORIGLAN,D2_VALCF3,D2_VALIPI,D2_VALACRS,D2_PICM,D2_PDV,D2_SEGUM,D2_QTSEGUM %Exp:cField% 
						FROM %Table:SD2% SD2
						WHERE
...
/*
					...
					SA5->(dbSetOrder(1))
					If SA5->(dbSeek(xFilial("SA5")+(cAliasSD2)->D2_CLIENTE + (cAliasSD2)->D2_LOJA + (cAliasSD2)->D2_COD))
						_cUnidMed := SA5->A5_YTPVEND  // 1- PRIMEIRA UNIDADE DE MEDIDA
					Endif
*/					
		            _cUnidMed := "1"         // 1 - PRIMEIRA UNIDADE DE MEDIDA
					
					SA7->(dbSetOrder(1))
					If SA7->(dbSeek(xFilial("SA7")+(cAliasSD2)->D2_CLIENTE + (cAliasSD2)->D2_LOJA + (cAliasSD2)->D2_COD))
						_cUnidMed := SA7->A7_YTPVEND  // 2- SEGUNDA UNIDADE DE MEDIDA
					Endif

					If !Empty((cAliasSD2)->D2_SEGUM) .And. (cAliasSD2)->D2_QTSEGUM > 0
			            If _cUnidMed == "2"  // 2 - SEGUNDA UNIDADE DE MEDIDA
				            _cUM	:= (cAliasSD2)->D2_SEGUM
				            _nQtde	:= (cAliasSD2)->D2_QTSEGUM
				 		Endif
					Endif
					...
					If lNfCup .Or. (cAliasSD2)->D2_ORIGLAN == "VD"
						...
						If SF2->F2_DESCONT > 0
							...
							Else			// Faz o rateio do desconto no total + o desconto no item
								nDesconto := (((_nQtde*(cAliasSD2)->D2_PRUNIT)/SF2->F2_VALMERC) * (SF2->F2_DESCONT- nTDescIt))+(cAliasSD2)->D2_DESCON 
							EndIf
						EndIf
		            Else 
						nDesconto := (cAliasSD2)->D2_DESCON            	
					...
					aadd(aProd,	{Len(aProd)+1,;
						...
						IIF(Empty(SB5->B5_UMDIPI),_cUM,SB5->B5_UMDIPI),;
						IIF(Empty(SB5->B5_CONVDIP),_nQtde,SB5->B5_CONVDIP*_nQtde),;
						(cAliasSD2)->D2_VALFRE,;
						(cAliasSD2)->D2_SEGURO,;
						(nDesconto+nDescIcm+nDescRed),;
						RetPrvUnit(cAliasSD2,nDesconto,_cUnidMed),;
		...
		Else				
			...
			#IFDEF TOP
				...
				BeginSql Alias cAliasSD1			
						...
						D1_NUMLOTE,D1_CUSTO,D1_ORIGLAN,D1_DESCICM,D1_II,D1_FORMUL,D1_VALPS3,D1_ORIGLAN,D1_VALCF3,D1_TESACLA,D1_IDENTB6,D1_PICM  %Exp:cField%
						FROM %Table:SD1% SD1
						WHERE
...
ElseIf _cEst == "EX"
	...
	cString += '<Exportador>'+"POLIMIX"+ '</Exportador>'
...
If  !lIssQn    
	...
	Elseif Len(aICMUFDest) == 0 .and. (cIdDest == "2" .and. cIndFinal == "1" .and. cIndIEDest == "9") .and. (Len(aISSQN)== 0) .and. (cAmbiente == "2" .or. (cAmbiente == "1" .and. FsDateConv(aNota[03],"YYYY") == "2016"))
...
//	cString += NfeTag('<locdespacho>'+Alltrim(SM0->M0_CIDCOB))
	cString += '</exporta>'	
Endif
...
/*/
//-----------------------------------------------------------------------
Static Function RetPrvUnit(cAliasSD2,nDesconto,_cUnidMed)

Local nRetorno := 0

Local _cUM		:= SB1->B1_UM
Local _nQtde	:= (cAliasSD2)->D2_QUANT
	...
	If !(lMvNFLeiZF)
		If _nQtde < 1		    
			if (cAliasSD2)->D2_PRUNIT == (cAliasSD2)->D2_PRCVEN
				nRetorno := ( ( ( (cAliasSD2)->D2_PRUNIT * _nQtde ) + nDesconto + (cAliasSD2)->D2_DESCZFR ) / _nQtde )	
			elseif (cAliasSD2)->D2_PRUNIT < (cAliasSD2)->D2_PRCVEN
				nRetorno := ( ( ( (cAliasSD2)->D2_PRCVEN * _nQtde ) + (cAliasSD2)->D2_DESCZFR ) / _nQtde )		
			else
				nRetorno := ( ( ( (cAliasSD2)->D2_PRUNIT * _nQtde ) + (cAliasSD2)->D2_DESCZFR ) / _nQtde )
			endif					
		Else
			nRetorno := ( ( (cAliasSD2)->D2_TOTAL + nDesconto + (cAliasSD2)->D2_DESCZFR ) / _nQtde )		
		Endif
	Else
		If _nQtde < 1		    
			if (cAliasSD2)->D2_PRUNIT == (cAliasSD2)->D2_PRCVEN
				nRetorno := ( ( ( (cAliasSD2)->D2_PRUNIT * _nQtde ) + nDesconto + (cAliasSD2)->D2_DESCZFR - ((cAliasSD2)->D2_DESCZFP + (cAliasSD2)->D2_DESCZFC)  ) / _nQtde )	
			elseif (cAliasSD2)->D2_PRUNIT < (cAliasSD2)->D2_PRCVEN
				nRetorno := ( ( ( (cAliasSD2)->D2_PRCVEN * _nQtde ) + (cAliasSD2)->D2_DESCZFR - ((cAliasSD2)->D2_DESCZFP + (cAliasSD2)->D2_DESCZFC) ) / _nQtde )		
			else
				nRetorno := ( ( ( (cAliasSD2)->D2_PRUNIT * _nQtde ) + (cAliasSD2)->D2_DESCZFR - ((cAliasSD2)->D2_DESCZFP + (cAliasSD2)->D2_DESCZFC)  ) / _nQtde )
			endif					
		Else
			nRetorno := ( ( (cAliasSD2)->D2_TOTAL + nDesconto + (cAliasSD2)->D2_DESCZFR - ((cAliasSD2)->D2_DESCZFP + (cAliasSD2)->D2_DESCZFC)  ) / _nQtde )	
		Endif
	Endif
