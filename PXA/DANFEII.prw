...
#DEFINE MAXMENLIN  095                                                // Máximo de caracteres por linha de dados adicionais
...
User Function PrtNfeSef(cIdEnt,cVal1,cVal2,oDanfe,oSetup,cFilePrint)
	...
	ElseIf oSetup:GetProperty(PD_PRINTTYPE) == IMP_PDF
		...
		IIF( !ExistDir('C:\DANFE'),MakeDir("C:\DANFE"),NIL)

		oDanfe:cPathPDF :=IIF(UPPER(Alltrim(oSetup:aOptions[PD_VALUETYPE]))=="C:\","C:\DANFE\", UPPER(Alltrim(oSetup:aOptions[PD_VALUETYPE])) )
...
Static Function PrtDanfe(oDanfe,oNFE,cCodAutSef,cModalidade,oNfeDPEC,cCodAutDPEC,cDtHrRecCab,dDtReceb,aNota)
	...
	*/
	...
	U_VN005A(.F.,.T.,.T.,SF2->F2_SERIE,SF2->F2_DOC,SF2->F2_DOC," ","ZZ")
//	U_VN005A(.F.,.T.,.T.,MV_PAR03,MV_PAR01,MV_PAR02," ","ZZ")

	SD2->(dbSetOrder(3))
	If SD2->(MsSeek(xFilial("SD2")+SF2->F2_DOC+SF2->F2_SERIE+SF2->F2_CLIENTE+SF2->F2_LOJA))

		SC6->(dbSetOrder(1))
		If SC6->(MsSeek(xFilial("SC6")+SD2->D2_PEDIDO))
			...
			While SC6->(!EOF()) .And. _cNum == SC6->C6_NUM
				...
				If SC6->C6_YPESLIQ > 0
					U_VN0003( SC6->C6_NUM, "SC5",SC6->C6_ITEM)
				Endif
