...
Autor		:
Data		:
Descrição	: Gerar o relatório de Ticket Balança
*/

User Function VN0003(p_cNumero, p_cwAlCab,p_cItem)
	...
	Private CONTFL      := 01
	Private m_pag       := 01
	Private cPerg 	    := "VN0003"
	Private wnrel       := "VN0003" // Coloque aqui o nome do arquivo usado para impressao em disco
	
	Private cNumero:= iif( p_cNumero<>nil, p_cNumero,'')
	Private cItem:= iif( p_cItem<>nil, p_cItem,If(p_cwAlCab == 'SC5', U_CheckItC6('2','C','RELATORIO'),'01'))
...
Static Function RELGRAF2(p_cNumero,p_cwAlCab,p_cItem)
	...
	case p_cwAlCab == 'SC5'
		...
		_cHrIni	  := _cHrFin := ''
		_nTpPsI   := If(Empty(SC6->C6_YTPPSIN),'',If(SC6->C6_YTPPSIN = "A","(Automático)","(Manual)"))
		_nTpPsF   := If(Empty(SC6->C6_YTPPSFI),'',If(SC6->C6_YTPPSFI = "A","(Automático)","(Manual)"))
	...
	case p_cwAlCab == 'SZH'
		...
		_nTpPsI   := If(Empty(SZH->ZH_TPPSINI),'',If(SZH->ZH_TPPSINI = "A","(Automático)","(Manual)"))
		_nTpPsF   := If(Empty(SZH->ZH_TPPSFIM),'',If(SZH->ZH_TPPSFIM = "A","(Automático)","(Manual)"))
...
//		nLin+=105
		...
		oPrn:Say( nLin+60 ,1600,"TICKET Balança ",oFont14N)
		nlin-=10
		...
		oPrn:Box(nLin ,0030, nLin + 0400,nColFin) //quadro do cabeçalho
...
//		nLin+=20
		...
		If p_cwAlCab == 'SZH'
			oPrn:Say(nLin  ,1550, "Hora Saída:" ,oFont14N)
		...
		oPrn:Say(nLin ,0600,_nTpPsI									,oFont14)
		...
		oPrn:Say(nLin ,0600,_nTpPsF									,oFont14)
		oPrn:Say(nLin ,1400,"Peso Líquido"							,oFont14)
		...
		oPrn:Say(nLin ,0050,"Peso Líquido"							,oFont14N)
...
//		nLin+= 100
		...
		oPrn:Box(nLin ,0030,nLin + 200,nColFin) //quadro de observações
		
		nLin+=40
		
		oPrn:Say(nLin ,0040,"Observação:  ",oFont14N)
		...
		oPrn:Say(nLin  ,0220,"Assinatura da Expedição",oFont14N)
