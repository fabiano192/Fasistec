		...
		Else
			...
			If !SF2->F2_TIPO $ "DB" 
				...
				If cMVNFEMSA1=="C" .And. !Empty(SA1->A1_MENSAGE)
					cMensCli	:=	Formula(SA1->A1_MENSAGE)
				ElseIf cMVNFEMSA1=="F" .And. !Empty(SA1->A1_MENSAGE)
					cMensFis	:=	Formula(SA1->A1_MENSAGE)+"#"
				EndIf
				aadd(aDest,AllTrim(SA1->A1_CGC))
				aadd(aDest,SA1->A1_NOME)
			...
			Else
				...
				If "/" $ MyGetEnd(SA2->A2_END,"SA2")[3]
					aadd(aDest,IIF(MyGetEnd(SA2->A2_END,"SA2")[3]<>"",MyGetEnd(SA2->A2_END,"SA2")[3],"SN"))
				Else
					aadd(aDest,IIF(MyGetEnd(SA2->A2_END,"SA2")[2]<>0,MyGetEnd(SA2->A2_END,"SA2")[2],"SN"))
				EndIf
			...
			While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
				...
				If !lNatOper
					If Empty(cNatOper)
						cNatOper := Alltrim((cAliasSD2)->D2_CF)
					Else
						cNatOper += Iif(!Alltrim((cAliasSD2)->D2_CF)$cNatOper,"/ " + (cAliasSD2)->D2_CF,"")
					Endif 
				Else	
					dbSelectArea("SX5")
					dbSetOrder(1)
				...
				If !Empty(aDest[15]) .And. !SubStr((cAliasSD2)->D2_CLASFIS,1,1)$"12"
				...
				SZI->(dbSetOrder(1))
				If SZI->(dbSeek(xFilial("SZI")+(cAliasSD2)->D2_TES))
					cMensFis += AllTrim(&(SZI->ZI_TEXTO))
//					cMensFis += AllTrim(SZI->ZI_TEXTO)
				Endif
				...
				If !Empty(SF4->F4_FORMULA) .And. Formula(SF4->F4_FORMULA) <> NIL .And. ( ( cMVNFEMSF4=="C" .And. !AllTrim(Formula(SF4->F4_FORMULA)) $ cMensCli ) .Or. (cMVNFEMSF4=="F" .And. !AllTrim(Formula(SF4->F4_FORMULA))$cMensFis) )
					...
					If cMVNFEMSF4=="C"
						If Len(cMensCli) > 0 .And. SubStr(cMensCli, Len(cMensCli), 1) <> " "
							cMensCli += " "
						EndIf
						cMensCli	+=	Formula(SF4->F4_FORMULA)
					ElseIf cMVNFEMSF4=="F"
						If Len(cMensFis) > 0 .And. SubStr(cMensFis, Len(cMensFis), 1) <> " "
							cMensFis += " "
						EndIf
...
//					IIf(Val(SB1->B1_CODBAR)==0,"",Str(Val(SB1->B1_CODBAR),Len(SB1->B1_CODBAR),0)),;				
				aadd(aProd,	{Len(aProd)+1,;
					...
					SF4->F4_AGREG}) // Tipo de agregaÁ„o de valor ao total do documento
				...
				If SFT->(FieldPos("FT_CSTPIS")) > 0 .And. SFT->(FieldPos("FT_CSTCOF")) > 0
					...
					If MsSeek(xFilial("SFT")+"S"+SF2->F2_SERIE+SF2->F2_DOC+SF2->F2_CLIENTE+SF2->F2_LOJA+PadR((cAliasSD2)->D2_ITEM,4)+(cAliasSD2)->D2_COD)
						...
						IF Empty(aPis[Len(aPis)]) .And. SFT->FT_CSTPIS$"09,06,04"
							aTail(aPisAlqZ):= {SFT->FT_CSTPIS}
						EndIf
						IF Empty(aCOFINS[Len(aCOFINS)]) .And. SFT->FT_CSTCOF$"09,06,04"
				...
				Else
					...
					IF Empty(aPis[Len(aPis)]) .And. SF4->F4_CSTPIS$"09,06,04"
						aTail(aPisAlqZ):= {SF4->F4_CSTPIS}		
					EndIf
					IF Empty(aCOFINS[Len(aCOFINS)]) .And. SF4->F4_CSTCOF$"09,06,04"
	...
	If MsSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
		...
		Else				
			...
			Else
				...
				aadd(aDest,SA2->A2_EMAIL)
		        /*
				If SF1->(FieldPos("F1_FORENT"))<>0 .And. !Empty(SF1->F1_FORENT+SF1->F1_LOJENT) .And. SF1->F1_FORENT+SF1->F1_LOJENT<>SF1->F1_FORNECE+SF1->F1_LOJA
				    dbSelectArea("SA2")
					dbSetOrder(1)
					MsSeek(xFilial("SA2")+SF1->F1_FORENT)
			...
			#IFDEF TOP
				...
				If SF1->(FieldPos("F1_STATUS")) > 0 .And. SD1->(FieldPos("D1_TESACLA"))> 0 .And. SF1->F1_STATUS='C' 
					BeginSql Alias cAliasSD1			
						...
						D1_LOTECTL,D1_NUMLOTE,D1_CUSTO
						FROM %Table:SD1% SD1
						WHERE
				...
				Else
					BeginSql Alias cAliasSD1
						...
						D1_SEGURO,D1_DESPESA,D1_CODISS,D1_VALISS,D1_VALIPI,D1_ICMSRET,D1_VUNIT,D1_CLASFIS,
						D1_VALICM,D1_TIPO_NF,D1_PEDIDO,D1_ITEMPC,D1_VALIMP5,D1_VALIMP6,D1_BASEIRR,D1_VALIRR,D1_TESACLA
						D1_TESACLA,D1_LOTECTL,D1_NUMLOTE,D1_CUSTO
						FROM %Table:SD1% SD1
						WHERE
						SD1.D1_FILIAL = %xFilial:SD1% AND
						SD1.D1_SERIE = %Exp:SF1->F1_SERIE% AND
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				If !lNatOper
					If Empty(cNatOper)
						cNatOper := Alltrim((cAliasSD1)->D1_CF)
					Else
						cNatOper += Iif(!Alltrim((cAliasSD1)->D1_CF)$cNatOper,"/ " + (cAliasSD1)->D1_CF,"")
					Endif 
				Else
					dbSelectArea("SX5")
					dbSetOrder(1)
				...
				If !Empty((cAliasSD1)->D1_NFORI) 
					...
					Else
						...
						If MsSeek(xFilial("SD2")+(cAliasSD1)->D1_NFORI+(cAliasSD1)->D1_SERIORI+(cAliasSD1)->D1_FORNECE+(cAliasSD1)->D1_LOJA+(cAliasSD1)->D1_COD+(cAliasSD1)->D1_ITEMORI)
							...
							//≥Obtem os dados de nota fiscal de produtor rural referenciada                                          |
						    //|Quando for uma NF de saida, o emitente serah sempre o sigamat                                         ≥
							//¿ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ|
							If AllTrim(SF2->F2_ESPECIE)=="NFP"
								//para nota de saida o emitente eh o proprio sigamat
								aadd(aNfVincRur,{SD2->D2_EMISSAO,SD2->D2_SERIE,SD2->D2_DOC,SF2->F2_ESPECIE,;
										SM0->M0_CGC,SM0->M0_ESTENT,SM0->M0_INSC})
							Endif
							// ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ|
				...
				If lEasy  .And. !Empty((cAliasSD1)->D1_TIPO_NF)
					...
					Else
						...
						//¿ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ
						aadd(aDI,(GetNFEIMP(.F.,cNFOri,cSerOri,cFornece,cLojaEnt,cTipoNFEnt, ,alltrim(Str(VAL(cItemOri))))))
					EndIf
					aAdi := aDI
				...
				ElseIf !lEasy
					...
					If MsSeek(xFilial("CD5")+(cAliasSD1)->D1_DOC+(cAliasSD1)->D1_SERIE+(cAliasSD1)->D1_FORNECE+(cAliasSD1)->D1_LOJA)
						// Pega todas as adiÁıes em sequÍncia correta
						While !CD5->(Eof()) .And. CD5->CD5_DOC == (cAliasSD1)->D1_DOC .And. CD5->CD5_SERIE == (cAliasSD1)->D1_SERIE .And. CD5->CD5_FORNEC == (cAliasSD1)->D1_FORNECE .And. CD5->CD5_LOJA == (cAliasSD1)->D1_LOJA
							aAdd(aDI,{;
								...
								{"I26","nAdicao",CD5->CD5_NADIC},;
								{"I27","nSeqAdi",CD5->CD5_SQADIC},;
...
If !Empty(aNota)
	...
	For nX := 1 To Len(aProd)
		cString += NfeItem(aProd[nX],aICMS[nX],aICMSST[nX],aIPI[nX],aPIS[nX],aPISST[nX],aCOFINS[nX],aCOFINSST[nX],aISSQN[nX],aCST[nX],aMed[nX],aArma[nX],aveicProd[nX],aDI[nX],aAdi[nX],aExp[nX],aPisAlqZ[nX],aCofAlqZ[nX],aAnfI[nX],cTipo,cVerAmb, aComb[Nx],@cMensFis,aCsosn[Nx],aPedCom[nX])
	Next nX
cString += NfeTotal(aTotal,aRetido,aICMS,aICMSST)
	...
	cString += NfeInfAd(cMensCli,cMensFis,aPedido,aExp,cAnfavea,aMotivoCont,aNota,aNfVinc)
	cString += "</infNFe>"
EndIf
...
If !Empty(aNfVinc)
	...
	For nX := 1 To Len(aNfVinc)
		If cVerAmb<>"2.00" .Or.;
			(cVerAmb=="2.00" .And. aScan(aNfVincRur,{|aX| aX[4]==aNfVinc[nX][6] .And. aX[2]==aNfVinc[nX][2] .And. aX[3]==aNfVinc[nX][3] .And. aX[5]==aNfVinc[nX][4]})==0)
...
If cVerAmb== "2.00" 
	Do Case
		Case (!Empty(aNfVinc).And. !(aNota[5]$"NDB") .And. SF4->F4_AJUSTE <> "S")
	  		cTPNota:= "2"
		Case (aNota[5]) == "I" .AND. SF4->F4_AJUSTE == "S"
	   		cTPNota:= "3"
		OtherWise 
	  		cTPNota:= "1"
		EndCase
...
If cVerAmb== "2.00"
    ...
    cFoneDest += IIF(aTelDest[3] > 0,ConvType(aTelDest[3],11),"") // CÛdigo do Telefone  // alterado por sergio de 9 para 11
...
If SA1->A1_CONTRIB=="2" .And. ConvType(aDest[14]) <> "ISENTO"
	cString += '<IE>'+""+'</IE>'
Else
...
If !Empty(aRetirada) .And. Len(AllTrim(aRetirada[01]))==14
	cString := '<retirada>'
...
Static Function NfeItem(aProd,aICMS,aICMSST,aIPI,aPIS,aPISST,aCOFINS,aCOFINSST,aISSQN,aCST,aMed,aArma,aveicProd,aDI,aAdi,aExp,aPisAlqZ,aCofAlqZ,aAnfI,cTipo,cVerAmb,aComb,cMensFis,cCsosn,aPedCom)
...
If cVerAmb== "2.00"
	cString += '<vUnCom>'+ConvType(aProd[16],21,6)+'</vUnCom>'	
Else                                       
...
If cVerAmb== "2.00"
	cString += '<vUnTrib>'+ConvType(aProd[10]/aProd[12],21,6)+'</vUnTrib>'		
Else
...
ElseIf _cEst == "EX"
	...
	cString += '<Exportador>'+"MAXCHEM"+ '</Exportador>'
	...
	Else		
		...
		If Len(aIcms)>0
			...
			cString += '<CST>'+ConvType(aICMS[02])+'</CST>'
	   		...
	   		IF(aCST[1] $ '40,41,50' .And. cVerAmb== "2.00") 
	   			cString += '<vBC>'+'0'+'</vBC>'     
		...
		If aCST[01] = "60" .AND. cTipo=="1"		
			...
			If nBaseIcm > 0 .and. nValICM>0
		   		If Len(cMensFis) > 0 .And. SubStr(cMensFis, Len(cMensFis), 1) <> " "
					cMensFis += " "
				EndIf
				cMensFis += "Imposto Recolhido por SubstituiÁ„o - Contempla os artigos 273, 274, 313 do RICMS. Valor da Base de ST: R$ "+str(nBaseIcm,15,2)+" Valor de ICMS ST: R$ "+str(nValICM,15,2)+" "
	        EndIf
...
Else
	cString += '<Tributo>'
	cString += '<CST>'+(IIF(Len(aPisAlqZ)>0 .And. aPisAlqZ[01]$"09,06,04",aPisAlqZ[01],"08"))+'</CST>'
	cString += '<modBC></modBC>'
	cString += '<pRedBC></pRedBC>'
	cString += '<vBC>'+ConvType(0,15,2)+'</vBC>'
	cString += '<aliquota>'+ConvType(0,5,2)+'</aliquota>'
...
Else
	cString += '<Tributo>'
	cString += '<CST>'+(IIF(Len(aCofAlqZ)>0 .And. aCofAlqZ[01]$"09,06,04",aCofAlqZ[01],"08"))+'</CST>'
	cString += '<modBC></modBC>'
	cString += '<pRedBC></pRedBC>'
	cString += '<vBC>'+ConvType(0,15,2)+'</vBC>'
	cString += '<aliquota>'+ConvType(0,5,2)+'</aliquota>'
...
Static Function NfeInfAd(cMsgCli,cMsgFis,aPedido,aExp,cAnfavea,aMotivoCont,aNfSa,aNfVinc)
...
//	cString += '<UFEmbarq>'+ConvType(aExp[01][01][03])+ '</UFEmbarq>'
//	cString += '<locembarq>'+ConvType(aExp[01][02][03])+ '</locembarq>'
	cString += '<UFEmbarq>'+"SP"+ '</UFEmbarq>'
	cString += '<locembarq>'+"SAO PAULO"+ '</locembarq>'
	cString += '</exporta>'	
EndIf
If Len(aPedido)>0
	cString += '<compra>'
...
If Len (cSeek)>0 .AND. nBaseIcm==0 .AND. nValICM==0
	dbSelectArea("SFT")
	dbSetOrder(4)
