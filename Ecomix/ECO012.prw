...
±±ºPrograma  ³ ECO012   º Autor ³ Alexandro          º Data ³  13/07/05   º±±
...
//Private _lSched := .T.  // ALEXANDRO
	...
	CONOUT("ATUALIZANDO ZZD --> ECO012 - DATA: "+TIME())
	...
	If !_lSched
		...
		@ 85,128 BMPBUTTON TYPE 5 ACTION Pergunte("ECO012")
...
//Close(oDlg1)
	...
	If MV_PAR03 == 1
		If Empty(aSelFil)
			If FindFunction("AdmSelecFil")
				AdmSelecFil("ECO012",03,.F.,@aSelFil,"ZZD",.F.)
			Else
	...
	cQuery += "  AND ZZD_CODFIL " + ECO12FIL()

	MemoWrite("\cfglog\ECO012A.txt",cQuery)
...
//	cQry += " AND F2_DOC = '000055758' "
	cQry += " AND F2_FILIAL " + ECO12FIL()
	cQry += " ORDER BY F2_FILIAL,F2_EMISSAO,F2_SERIE,F2_DOC "

	MemoWrite("\CFGLOG\ECO012B.TXT",cqry)
	...
	While !QRYSF2->(Eof())
		...
		U_ECO013(_cfilial,cDoc,cSERIE,cCliente,cLoja,_cProduto)
	...
	If Alltrim(cEmpAnt)+Alltrim(cFilAnt) $ '1306|1307'
		...
		cQry += " AND C5_XOPER =  'V' "
		cQry += " AND C5_FILIAL " + ECO12FIL()
		cQry += " AND C5_CLIENTE = '000276' AND C5_NOTA = '' "
		cQry += " ORDER BY C5_FILIAL,C5_EMISSAO,C5_NUM "

		MemoWrite("\CFGLOG\ECO012E.TXT",cqry)
...
Static Function ECO12_01(_lSch)
...
//CONOUT("ATUALIZANDO ZZD --> ECO012 - DATA: "+STR(TIME()))
	...
	_aEmpresa := {"01","02","04","09","13","16","50"}
	...
	For AX:= 1 To Len(_aFiliais)
		...
		U_ECO12_02(_aFiliais[AX][1],_aFiliais[AX][2])
...
User Function ECO12_02(_cEmpresa,_cFilial)
...
//ProcRegua(QRYSF2->(U_CONTREG()))
	...
	MemoWrite("\CFGLOG\ECO012C.TXT",cqry)
