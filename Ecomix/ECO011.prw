...
Static Function ECO11_01C(_cEmp,_cFil,_cFilRM)
	...
	//_cQry += " AND NUMEROMOV = '000023249' " //C18728
	...
	Else
		...
		If EmpOpenFile("TSBM","SBM",1,.T., _cEmp,@_cModo)
			_cAliasSBM  := "TSBM"
			_lOutSBM := .T.
		Endif
		...
		While TRB->(!EOF())
			...
			While TRB->(!EOF())  .And. _cKey1 == Alltrim(cValToChar(TRB->CODCOLIGADA))
				...
				Else
					...
					_lRet := CheckZF6(1,_cFil,_cAliasZF6,_cAliasSA2,_cAliasSB1,_cAliasSA1,_cAliasSBM)
				...
				If (_F1DOC = '000036220' .And. TRB->IDMOV = 270637 .And. TRB->CODCOLIGADA = 9) .Or.;
						...
						(_F1DOC = '000000004' .And. TRB->IDMOV = 406195 .And. TRB->CODCOLIGADA = 11) .or.;
						(_F1DOC = '000045990' .And. TRB->IDMOV = 456459 .And. TRB->CODCOLIGADA = 11)
				...
				While TRB->(!EOF())  .And. _cKey2 == Alltrim(cValToChar(TRB->CODCOLIGADA)) + Alltrim(cValToChar(TRB->IDMOV))
					...
					_lRet      := CheckZF6(2,_cFil,_cAliasZF6,_cAliasSA2,_cAliasSB1,_cAliasSA1,_cAliasSBM)

					_D1GRUPO   := (_cAliasSB1)->B1_GRUPO
...
Static Function CheckZF6(_nOpc,_cFil,_cAliasZF6,_cAliasSA2,_cAliasSB1,_cAliasSA1,_cAliasSBM)
	...
	If _nOpc = 1
		...
		If _lGrava
			...
			If !(_cAliasSA2)->(MsSeek(xFilial("SA2")+_cCNPJ))
				...
				If !(_cAliasSA2)->(MsSeek(xFilial("SA2")+cValToChar(TFOR->IDCFO)))
	...
	ElseIf _nOpc = 3
		...
		If _lGrava
			...
			Else
				...
				_cCodFor  := (_cAliasSA1)->A1_COD
				_cLojFor  := (_cAliasSA1)->A1_LOJA
			Endif
		...
		If _nReg > 0
			...
			*/
			If Empty(_D1COD)
				...
				_D1COD := Alltrim(TPROD->CODIGOPRD)

				(_cAliasSB1)->(dbOrderNickName("INDSB1"))
				If !(_cAliasSB1)->(MsSeek(xFilial("SB1") + _D1COD))

					GeraProd(_cFil,_D1COD,_cAliasSB1,_cAliasSBM)
...
Static Function GeraFOR(_cFil,_cCNPJ,_cCod,_cLoja,_cAliasSA2)
	...
	(_cAliasSA2)->A2_YIDRM   := cValToChar(TFOR->IDCFO)
...
Static Function GeraProd(_cFil,_cProd,_cAliasSB1,_cAliasSBM)

	_cGrProd := ""
	(_cAliasSBM)->(dbOrderNickName("INDSBM1"))
	If (_cAliasSBM)->(MsSeek(xFilial("SBM")+ Left(_cProd,5) ))
