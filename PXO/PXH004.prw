...
Static Function PX004_01(_lFim)
    ...
    _cQ := " SELECT * FROM "+RetSqlName("SE5")+" A INNER JOIN "+RetSqlName("SA6")+ " B "
    If cEmpAnt == "10"
        _cQ += " ON LEFT(E5_FILIAL,3)= A6_FILIAL AND E5_BANCO=A6_COD AND E5_AGENCIA=A6_AGENCIA AND E5_CONTA=A6_NUMCON "
    Else
        _cQ += " ON E5_FILIAL=A6_FILIAL AND E5_BANCO=A6_COD AND E5_AGENCIA=A6_AGENCIA AND E5_CONTA=A6_NUMCON "
    Endif
...
//_cQ += " ORDER BY E5_DTDISPO "
    ...
    MemoWrit("C:\TEMP\PXH004.TXT",_cQ)
			...
			*/
            ...
            ElseIf !Empty(ZZ->E5_FORNECE)
                ...
                If SA2->(dbSeek(xFilial("SA2")+ZZ->E5_FORNECE + ZZ->E5_LOJA))
                    ...
                    Endif
                Endif
            ElseIf !Empty(ZZ->E5_TIPO) .And. Empty(ZZ->E5_MOEDA)
                _cHist  := Lower(Alltrim(ZZ->E5_HISTOR))
            Endif
