...
*/
User Function PXH018(_cfilial,cDoc,cSERIE,cCliente,cLoja,_cProduto)
...
Static Function GERAMOVS()
	...
	Else
		...
		ZZD->ZZD_ICMS	:= IF (QRYSF2->D2_VALISS > 0, 0, QRYSF2->D2_VALICM)	// ICMS
		If ZZD->(FieldPos("ZZD_VALISS")) > 0
	...
	*/
	...
	cSigla:= SuperGetMv( "PXO_SIGLA" , .F. ,'' , QRYSF2->F2_FILIAL ) //  getNewPar('BRI_SIGLA','BRI ')
...
Static Function fPesqSA1(cTipo)
	...
	If cTipo == "SF2"
		If !QRYSF2->F2_TIPO $ "B/D"

			DbSelectArea("SA1")
			aArea := GetArea()
			DbSetOrder(1)
			If MsSeek(xFilial("SA1") + QRYSF2->F2_CLIENTE + QRYSF2->F2_LOJA)
				Private aCliente:= {}
		...
		Else
			...
			aArea := GetArea()
			dbSetOrder(1)
			If MsSeek(xFilial("SA2") + QRYSF2->F2_CLIENTE + QRYSF2->F2_LOJA)
				Private aCliente:= {}
				Private cPriCom:= ""//Substr(DTOS(SA1->A1_PRICOM),5,2)+"/"+Substr(DTOS(SA1->A1_PRICOM),1,4)
...
Static Function fPesqSD2()
	...
	If SD2->(FieldPos("D2_YPRG")) > 0
		If QRYSF2->D2_YPRG > 0
			//		If cEmpant == "02" .And. cFilAnt == "11" .And. QRYSF2->F2_EMISSAO < CTOD("01/08/2017")
			//			_nPrc := QRYSF2->D2_YPRG / SuperGetMV("PXO_FATCON")
...
Static Function GERAMOVE()
	...
	*/
	...
	cSigla:= SuperGetMv( "PXO_SIGLA" , .F. ,'', ZZ->D1_FILIAL )
...
Static Function fPesqSD1()
	...
	If SD2->(FieldPos("D2_YPRG")) > 0
		If SD2->D2_YPRG > 0
			...
			//			_nPrc := SD2->D2_YPRG / SuperGetMV("PXO_FATCON")
			//			aAdd(aNota, _nPrc)		    	// 8 Preco Gerencial
			//		Else
